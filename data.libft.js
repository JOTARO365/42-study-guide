...
ในเวิร์กสเปซนี้มีของ 2 โปรเจกต์อื่นวางไว้ในโฟลเดอร์เดียวกันด้วย:
  ...
  get_next_line.c  get_next_line_utils.c  ...h       → หน้า get_next_line`, cap: "3 โปรเจกต์แยกกันตอนส่ง แต่รวมโฟลเดอร์เพื่อให้โปรเจกต์ปลายทางลิงก์ไฟล์เดียว", lang: "txt" },
      ...
      { h: "Makefile ตั้งแต่ศูนย์ — ใช้ซ้ำได้ทุกโปรเจกต์หลังจากนี้" },
      { p: "libft คือที่แรกที่ต้องเขียน Makefile และเป็นตัวที่จะถูกก๊อปไปใช้ต่อใน push_swap, pipex, so_long, fdf, minishell, cub3D ทุกตัว. ลงทุนทำความเข้าใจตรงนี้ครั้งเดียวแล้วไม่ต้องกลับมางงอีก" },
      { h: "1) make ทำงานยังไง (แนวคิดเดียวที่ต้องเข้าใจ)" },
      { p: "make ไม่ได้ทำงานเป็นสคริปต์จากบนลงล่าง. มันดู **rule **ที่เขียนไว้ว่า** `เป้าหมาย: สิ่งที่ต้องมีก่อน` **แล้วถามคำถามเดียว:** ไฟล์เป้าหมายเก่ากว่าสิ่งที่มันขึ้นอยู่ด้วยหรือเปล่า** ถ้าเก่ากว่าค่อยรันคำสั่งข้างล่าง ถ้าไม่ก็ข้าม. ทั้งเรื่อง 'ไม่ relink' คือผลของกฎข้อนี้ล้วน ๆ" },
      { code: String.raw`เป้าหมาย: สิ่งที่ต้องมีก่อน สิ่งที่ต้องมีก่อน
<TAB>คำสั่งที่จะรัน

libft.a: ft_strlen.o ft_strdup.o        <- libft.a เก่ากว่า .o ตัวไหนไหม
	ar rcs libft.a ft_strlen.o ft_strdup.o

ft_strlen.o: ft_strlen.c                <- .o เก่ากว่า .c ไหม
	cc -Wall -Wextra -Werror -c ft_strlen.c`, cap: "บรรทัดคำสั่งต้องขึ้นต้นด้วย TAB จริง ๆ ไม่ใช่ space — ผิดตรงนี้จะได้ error ว่า missing separator", lang: "make" },
      { note: "**นี่คือ error แรกที่ทุกคนเจอ** — make จะบอกว่า missing separator ที่บรรทัดนั้น แปลว่าบรรทัดคำสั่งใช้ space แทน TAB. editor ที่ตั้ง expand-tab ไว้จะทำให้เกิดเรื่องนี้ทุกครั้ง — ตั้งให้ไฟล์ชื่อ Makefile ใช้ TAB จริง" },
      { h: "2) ตัวแปร: = กับ := ต่างกันจริง" },
      { code: String.raw`NAME    = libft.a          # = ประเมินตอนถูกใช้ (recursive)
CC      = cc
CFLAGS  = -Wall -Wextra -Werror
AR      = ar rcs
RM      = rm -f

SRCS    := ft_strlen.c ft_strdup.c    # := ประเมินทันทีตอนอ่านบรรทัดนี้
OBJS    = $(SRCS:.c=.o)               # แทนที่นามสกุล: .c -> .o`, cap: "ใช้ := เมื่อค่าไม่ควรเปลี่ยนอีก, ใช้ = เมื่ออยากให้ค่าถูกคำนวณตอนใช้จริง — ในไฟล์ขนาดนี้ต่างกันน้อย แต่ควรรู้ว่ามันคนละอย่าง", lang: "make" },
      { p: "`$(SRCS:.c=.o)` คือ **substitution reference** — วิธีสร้างรายชื่อ `.o` จากรายชื่อ `.c` โดยไม่ต้องพิมพ์ซ้ำ. เพิ่มไฟล์ใหม่แก้ที่เดียวคือ `SRCS`" },
      { h: "3) ตัวแปรอัตโนมัติ — $@ $< $^" },
      { table: { head: ["ตัวแปร", "หมายถึง", "ตัวอย่างในบริบท"], rows: [
        ["`$@`", "ชื่อเป้าหมายของ rule นี้", "ใน `libft.a: ...` ค่าคือ `libft.a`"],
        ["`$<`", "สิ่งที่ต้องมีก่อน **ตัวแรก**", "ใน pattern rule คือไฟล์ `.c` ที่กำลังคอมไพล์"],
        ["`$^`", "สิ่งที่ต้องมีก่อน **ทั้งหมด**", "รายชื่อ `.o` ทุกตัว"],
      ]}},
      { code: String.raw`%.o: %.c
	$(CC) $(CFLAGS) -I. -c $< -o $@

# บรรทัดเดียวนี้แทน rule ของทุกไฟล์ .c ในโปรเจกต์
# %  = wildcard ที่จับคู่กัน: ft_strlen.o มาจาก ft_strlen.c`, cap: "pattern rule — เหตุผลที่ Makefile ยาว 40 บรรทัดคุมไฟล์ 43 ไฟล์ได้", lang: "make" },
      { note: "**ต้องคอมไพล์ทีละไฟล์แบบนี้ ห้ามรวบเป็นบรรทัดเดียว** อย่าง `cc *.c -o prog`. ถ้ารวบ make จะไม่มีทางรู้ว่าไฟล์ไหนเปลี่ยน จึงคอมไพล์ใหม่หมดทุกครั้ง = relink = ตกข้อนี้ตอนตรวจ" },
      { h: "4) โครง Makefile ของ libft ทั้งไฟล์" },
...
SRCS    = ft_isalpha.c ft_isdigit.c ... ft_putnbr_fd.c    # 34 ไฟล์
OBJS    = $(SRCS:.c=.o)

SRCS_B  = ft_lstnew_bonus.c ... ft_lstmap_bonus.c         # 9 ไฟล์
OBJS_B  = $(SRCS_B:.c=.o)
...
.PHONY: all bonus clean fclean re`, cap: "40 บรรทัดนี้คือแม่แบบที่จะถูกดัดแปลงไปใช้ในทุกโปรเจกต์ถัดไป", lang: "make" },
      { h: "5) สามจุดที่คนพลาดบ่อยที่สุด" },
      { ul: [
        "**ไฟล์ .o ต้องขึ้นอยู่กับ header ด้วย** — เขียน `%.o: %.c libft.h` ไม่งั้นแก้ prototype ใน `libft.h` แล้ว make จะไม่คอมไพล์อะไรใหม่เลย และเราจะไล่บั๊กที่ไม่มีอยู่จริง",
        "**.PHONY จำเป็นจริง** — ถ้าบังเอิญมีไฟล์ชื่อ `clean` อยู่ในโฟลเดอร์ make จะเห็นว่า 'เป้าหมาย clean มีอยู่แล้วและใหม่กว่า' แล้วข้ามไปเฉย ๆ. `.PHONY` บอกว่าเป้าหมายพวกนี้ไม่ใช่ไฟล์",
        "**fclean ต้องเรียก clean ต่อ** เขียนเป็น `fclean: clean` แล้วค่อยลบ `$(NAME)` — ไม่ใช่ก๊อป `rm` มาซ้ำสองที่ซึ่งจะลืมอัปเดตทีหลัง",
      ]},
      { h: "6) พิสูจน์ว่าไม่ relink" },
...
$ make
make: Nothing to be done for 'all'.      <- ต้องได้บรรทัดนี้

$ touch libft.h && make                  <- แก้ header
cc ... -c ft_isalpha.c ...               <- ต้องคอมไพล์ใหม่ทุกไฟล์

$ make fclean && make re                 <- ต้องสร้างใหม่หมดโดยไม่ error`, cap: "ผู้ตรวจสั่ง make สองครั้งเป็นอย่างแรก — ครั้งที่สองที่ยังคอมไพล์อยู่คือจบเลย", lang: "bash" },
      { h: "7) ต่อยอดไปโปรเจกต์ถัดไป" },
      { table: { head: ["โปรเจกต์", "ต้องเพิ่มอะไรจากแม่แบบ"], rows: [
        ["push_swap · pipex · minitalk", "มีโฟลเดอร์ `libft/` ข้างใน จึงต้องมี rule ที่สั่ง `make -C libft` ก่อน แล้วลิงก์ `libft/libft.a` ตอนสร้างไบนารี · `clean`/`fclean` ต้อง `make -C libft` ตามไปด้วย"],
        ["so_long · fdf · fract-ol · cub3D · miniRT", "เพิ่มการคอมไพล์ MiniLibX และ flag ตอนลิงก์ `-lmlx -lXext -lX11 -lm`"],
        ["โปรเจกต์ที่มี bonus เป็นไบนารีคนละตัว", "แยก `SRCS_B` แล้วให้ `bonus` สร้างไบนารีอีกชื่อ — ต้องเขียนเป็น rule จริง (`$(BONUS_NAME): $(OBJS_B)`) ไม่ใช่แปะคำสั่งบน target `bonus` ตรง ๆ ไม่งั้นมัน relink ทุกครั้ง"],
        ["C++ modules", "เปลี่ยน `CC = c++` และ `CFLAGS = -Wall -Wextra -Werror -std=c++98` ที่เหลือโครงเดิม"],
      ]}},
      { code: String.raw`# แบบที่มี libft อยู่ข้างใน
LIBFT_DIR = libft
...
clean:
	$(RM) $(OBJS)
	make -C $(LIBFT_DIR) clean`, cap: "ห้ามคอมไพล์ซอร์สของ libft ซ้ำใน Makefile ตัวแม่ — ให้เรียก Makefile ของมันเอง", lang: "make" },
      { note: "ลำดับตอนลิงก์มีผล: `cc main.c libft.a` ทำงาน แต่ `cc libft.a main.c` อาจไม่ เพราะ linker เดินจากซ้ายไปขวาและดึงเฉพาะ symbol ที่ **ยังขาดอยู่ตอนที่มันเดินมาถึง** archive. ไลบรารีต้องอยู่ท้ายเสมอ" },
