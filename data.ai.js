...
→ เลือก temperature ตามงาน:
  ...
  เขียนนิยาย/ไอเดีย: 0.7-1.2`, lang: "txt" },
      ...
      { qa: [
        { q: "autoregressive decoding คืออะไร?", a: "โมเดลทำนายโทเคนถัดไปทีละตัว แล้วเอาตัวที่เพิ่งสร้างต่อท้ายอินพุตเพื่อทำนายตัวถัดไป — ข้อความทั้งย่อหน้าจึงเกิดจากการทำนายทีละก้าวซ้ำ ๆ ไม่ใช่การวางแผนล่วงหน้า" },
        { q: "temperature ทำอะไรกับการเลือกโทเคน?", a: "ปรับความชันของการแจกแจงความน่าจะเป็น — ค่าต่ำทำให้เลือกตัวที่น่าจะเป็นที่สุดเกือบตลอด (ผลซ้ำเดิม) ค่าสูงทำให้ตัวที่โอกาสน้อยมีสิทธิ์ถูกเลือกมากขึ้น (หลากหลายแต่เพี้ยนง่าย)" },
        { q: "ทำไมโมเดลถึง 'มั่ว' อย่างมั่นใจได้?", a: "เพราะมันเลือกโทเคนที่น่าจะเป็นตามรูปแบบภาษา ไม่ได้ตรวจสอบกับความจริง — ข้อความที่ผิดแต่เขียนถูกรูปแบบจึงมีความน่าจะเป็นสูงพอ ๆ กับข้อความที่ถูก" },
      ]},
...
→ ยิ่งอักขระ 'แปลก' ยิ่งกิน token เยอะ`, lang: "txt" },
      ...
      { qa: [
        { q: "BPE ทำงานยังไง?", a: "เริ่มจากตัวอักษรเดี่ยว แล้ววนรวมคู่ที่พบบ่อยที่สุดเป็นหน่วยใหม่ซ้ำ ๆ จนได้คลังโทเคนตามขนาดที่กำหนด — คำที่พบบ่อยจึงกลายเป็นโทเคนเดียว ส่วนคำหายากถูกแยกเป็นชิ้น" },
        { q: "ทำไมภาษาไทยกินโทเคนมากกว่าอังกฤษ?", a: "คลังโทเคนถูกฝึกจากข้อมูลที่เป็นภาษาอังกฤษเป็นหลัก — คำไทยจึงมักถูกตัดเป็นหลายชิ้นหรือแม้แต่ทีละไบต์ ทำให้ข้อความยาวเท่ากันใช้โทเคนมากกว่าหลายเท่า" },
        { q: "ทำไมโมเดลนับตัวอักษรในคำผิดบ่อย?", a: "เพราะมันไม่เห็นตัวอักษร — เห็นเป็นโทเคน. คำที่เป็นโทเคนเดียวไม่มีข้อมูลว่าข้างในมีตัวอักษรอะไรกี่ตัว" },
      ]},
...
พอจะเกิน → ต้อง:
  - ตัดประวัติเก่าทิ้ง (sliding window)
  - สรุปประวัติเป็นย่อ (summarization)
  - ดึงเฉพาะ docs ที่เกี่ยวจริง (RAG แทนยัดทั้งหมด)`, cap: "ทุก token นับรวมสู่เพดาน — 'ส่งทั้งหมดไป' ไม่ใช่ทางออก ต้องเลือกเฉพาะที่เกี่ยว", lang: "txt" },
      { p: "**ทำไม context ยาว = ช้าและแพงแบบทวีคูณ:** attention ของ transformer มีต้นทุน **O(n²)** ตามความยาว — context ยาว 2 เท่า ใช้ compute ~4 เท่า. ฉะนั้นยัด context เกินจำเป็น = จ่ายแพงขึ้นแบบกำลังสอง + ช้าลง" },
      { p: "**Lost in the middle — โมเดลลืมตรงกลาง:** งานวิจัยพบว่าโมเดล 'ใส่ใจ' ข้อมูลที่ **ต้นและท้าย** context ดีกว่าตรงกลาง. ข้อมูลสำคัญที่ฝังกลาง ๆ ของ prompt ยาว ๆ จะถูกดึงมาใช้แม่นน้อยลง" },
      { code: String.raw`การวางตำแหน่งใน prompt (จากถูกจำดี → แย่):
  ต้น context   ████████  ดีสุด
  ท้าย context  ███████   ดี (ใกล้คำถาม)
  กลาง context  ███       แย่ ('lost in the middle')
...
→ practice: วาง 'เอกสาร/คำสั่งสำคัญไว้บนสุด' แล้ว 'คำถามไว้ล่างสุด'
  RAG: เรียง chunk ที่เกี่ยวสุดไว้ใกล้ขอบ (ต้น/ท้าย) ไม่ใช่กลางกอง`, cap: "วางของสำคัญไว้ต้นหรือท้าย ไม่ใช่กลาง — ตรงกับทริค 'เอกสารยาวไว้บนสุด' ในแท็บทฤษฎี", lang: "txt" },
      ...
      { qa: [
        ...
        { q: "ควรวางคำถามไว้ตรงไหนของ prompt?", a: "ไว้ 'ล่างสุด' (ใกล้จุดที่โมเดลเริ่มตอบ) ส่วนเอกสาร/บริบทยาว ๆ ไว้บน — ลด lost-in-the-middle และให้โมเดลเห็นคำถามชัดก่อนตอบ" },
      ]},

      { qa: [
        { q: "context window คืออะไร?", a: "จำนวนโทเคนสูงสุดที่โมเดลรับได้ในคำขอเดียว นับรวมทั้งอินพุตและเอาต์พุต — เกินแล้วต้องตัดทิ้งหรือย่อ ไม่มีการจำข้ามคำขอโดยอัตโนมัติ" },
        { q: "lost in the middle คืออะไร?", a: "ปรากฏการณ์ที่โมเดลใช้ข้อมูลตอนต้นและตอนท้ายของ context ได้ดีกว่าตรงกลาง — เอกสารสำคัญจึงควรวางไว้ต้นหรือท้าย ไม่ใช่กลางกอง" },
        { q: "context ใหญ่ขึ้นแปลว่าดีขึ้นเสมอไหม?", a: "ไม่ — ยัดข้อมูลที่ไม่เกี่ยวเข้าไปทำให้ทั้งแพงขึ้นและแม่นยำลดลง. การคัดเฉพาะส่วนที่เกี่ยวข้อง (แบบที่ RAG ทำ) ให้ผลดีกว่าการยัดทุกอย่าง" },
      ]},
...
ได้อะไร:
  ...
  - โมเดลไม่ต้องเสีย token ไปกับการจัดรูปแบบเอง`, lang: "txt" },
      ...
      { qa: [
        { q: "structured output รับประกันอะไร?", a: "รับประกันว่าผลลัพธ์ตรงกับ schema ที่ให้ไว้ในเชิงรูปแบบ — ทำได้เพราะระบบจำกัดโทเคนที่เลือกได้ในแต่ละก้าวให้เหลือเฉพาะตัวที่ยังทำให้ JSON ถูกต้องตาม schema" },
        { q: "ต่างจากการสั่งใน prompt ว่า 'ตอบเป็น JSON' ยังไง?", a: "การสั่งใน prompt เป็นแค่คำขอ — โมเดลยังพลาดได้ทั้งวงเล็บเกิน ฟิลด์หาย หรือมีข้อความนำหน้า. การจำกัดตอนถอดรหัสทำให้ผลลัพธ์ที่ผิดรูปแบบ *เกิดขึ้นไม่ได้*" },
        { q: "ยังต้องตรวจผลลัพธ์อยู่ไหม?", a: "ต้อง — รูปแบบถูกไม่ได้แปลว่าเนื้อหาถูก. schema บังคับได้แค่ชนิดและโครงสร้าง ไม่ได้บังคับว่าค่าที่ใส่มาเป็นความจริง" },
      ]},
...
→ Caching คุ้มสุดเมื่อ:
  ...
  3. ไม่ใช่ batch processing (batch ลดราคา 50% อยู่แล้ว)`, cap: "Caching คุ้มจริงเฉพาะเมื่อ prompt ยาว + ใช้ซ้ำหลายรอบ — ถ้าส่งครั้งเดียวไม่คุ้ม", lang: "txt" },
      ...
      { qa: [
        { q: "prompt caching ทำงานยังไง?", a: "เก็บผลการประมวลผลของ *ส่วนนำหน้า* ที่ซ้ำกันไว้ — คำขอถัดไปที่ขึ้นต้นเหมือนกันเป๊ะจะข้ามการคำนวณส่วนนั้นและจ่ายในอัตราอ่านแคชซึ่งถูกกว่ามาก" },
        { q: "อะไรทำให้แคชไม่ติด?", a: "อะไรก็ตามที่เปลี่ยนส่วนนำหน้า — แม้แต่ช่องว่างเดียว, วันเวลาที่ใส่ไว้ต้น prompt, หรือการสลับลำดับเครื่องมือ. ส่วนที่เปลี่ยนต้องอยู่ **ท้าย** เสมอ" },
        { q: "จุดคุ้มทุนของแคชอยู่ตรงไหน?", a: "เขียนแคชแพงกว่าราคาปกติ ส่วนอ่านถูกกว่ามาก — จึงคุ้มเมื่อส่วนนำหน้าเดิมถูกใช้ซ้ำเกินจำนวนครั้งหนึ่ง. ยืนยันด้วยการดูจำนวนโทเคนที่อ่านจากแคชในผลลัพธ์จริง ไม่ใช่เดา" },
      ]},
...
→ loop: model ขอ tool เพิ่มได้หลายรอบก่อนตอบสุดท้าย (= agent loop)`, cap: "model วนขอ tool ได้หลายรอบ (และขนานได้) ก่อนสรุป — นี่คือแกนของ ReAct/agent", lang: "txt" },
      ...
      { qa: [
        { q: "tool calling ทำงานเป็นวงจรยังไง?", a: "โมเดลตอบกลับมาว่าจะเรียกเครื่องมืออะไรพร้อม argument → **โค้ดของเราเป็นคนรัน** → ส่งผลลัพธ์กลับเข้าไปเป็นข้อความถัดไป → โมเดลใช้ผลนั้นตอบต่อ. วนซ้ำจนไม่มีการเรียกเครื่องมืออีก" },
        { q: "โมเดลรันเครื่องมือเองไหม?", a: "ไม่ — มันแค่บอกว่าอยากเรียกอะไร. การรันจริงอยู่ฝั่งเรา ซึ่งเป็นจุดที่ต้องใส่การตรวจสิทธิ์และการจำกัดขอบเขต" },
        { q: "คำอธิบายเครื่องมือสำคัญแค่ไหน?", a: "สำคัญมาก — เป็นข้อมูลเดียวที่โมเดลใช้ตัดสินว่าจะเรียกตัวไหนและใส่ argument อะไร. คำอธิบายกำกวมทำให้เลือกผิดเครื่องมือบ่อยกว่าปัญหาอื่นทั้งหมดรวมกัน" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
→ สำหรับ RAG: ใช้ cosine similarity (หรือ dot product หลัง normalize)`, lang: "txt" },
      ...
      { qa: [
        { q: "cosine similarity วัดอะไร?", a: "วัดมุมระหว่าง 2 เวกเตอร์ ไม่ใช่ระยะห่าง — ค่าอยู่ระหว่าง -1 ถึง 1 โดย 1 คือทิศเดียวกันสนิท. เอกสารยาวกับสั้นที่พูดเรื่องเดียวกันจึงยังใกล้กันได้" },
        { q: "ทำไมไม่ใช้ระยะแบบยุคลิดตรง ๆ?", a: "เพราะขนาดของเวกเตอร์ผูกกับความยาวข้อความมากกว่าความหมาย — วัดมุมจึงตัดผลของความยาวออกไป. ถ้าเวกเตอร์ถูกทำให้ยาว 1 หน่วยแล้ว ทั้งสองวิธีให้ลำดับเดียวกัน" },
        { q: "ค่า 0.85 แปลว่าใกล้เคียงพอไหม?", a: "ขึ้นกับโมเดลและชุดข้อมูล — ไม่มีเกณฑ์สากล. ต้องหาเกณฑ์จากชุดทดสอบของตัวเองโดยดูว่าจุดตัดไหนแยกผลที่เกี่ยวข้องออกจากที่ไม่เกี่ยวได้ดีที่สุด" },
      ]},
...
ผลจริง:
  ...
  - HNSW index:     ~5ms (400x เร็วขึ้น)`, cap: "pgvector + HNSW ทำให้ vector search เร็วพอสำหรับ production โดยไม่ต้องมี DB แยก", lang: "txt" },
      ...
      { qa: [
        { q: "HNSW ทำให้ค้นเร็วขึ้นได้ยังไง?", a: "สร้างกราฟหลายชั้น — ชั้นบนมีจุดน้อยและเชื่อมข้ามระยะไกล ใช้กระโดดเข้าใกล้เป้าหมายอย่างหยาบ ๆ ก่อน แล้วค่อยลงชั้นล่างที่ละเอียดขึ้นเพื่อปรับให้แม่น" },
        { q: "ผลที่ได้เป็นคำตอบที่ถูกต้องเสมอไหม?", a: "ไม่ — เป็นการค้นแบบ *ประมาณ*. แลกความแม่นยำเล็กน้อยกับความเร็วที่มากขึ้นหลายร้อยเท่า และปรับสมดุลได้ด้วยพารามิเตอร์ของดัชนี" },
        { q: "เมื่อไหร่ไม่ควรใช้ ANN?", a: "เมื่อข้อมูลน้อย (หลักพันถึงหมื่น) — การไล่เทียบทั้งหมดยังเร็วพอและได้คำตอบที่ถูกต้องแน่นอน โดยไม่ต้องดูแลดัชนีเพิ่ม" },
      ]},
...
int8    (1 byte):  1M × 1536 × 1  = 1.5 GB   (เล็กลง 4 เท่า)
binary  (1 bit):   1M × 1536 / 8  = 192 MB   (เล็กลง 32 เท่า!)`, cap: "ตัวเลขใน vector ไม่ต้องเก็บ 32-bit เต็มเสมอ — ลดบิตต่อตัว = ย่อ storage ตรง ๆ", lang: "txt" },
      { p: "**ทาง 1 — ลดมิติ (Matryoshka):** embedding model รุ่นใหม่ (เช่น OpenAI text-embedding-3, Gemini) ฝึกแบบ Matryoshka ให้ 'มิติต้น ๆ บรรจุความหมายส่วนใหญ่' → ตัดท้ายทิ้งได้ (1536→256) โดยความแม่นลดนิดเดียว. สั่งผ่าน `dimensions` / `output_dimensionality`" },
      { p: "**ทาง 2 — Quantization (ลดบิตต่อตัวเลข):** เก็บแต่ละมิติด้วยบิตน้อยลง:" },
      { code: String.raw`float32 → int8 (scalar quantization):
  แปลงช่วง float เป็นจำนวนเต็ม 0-255 ต่อมิติ → เล็กลง 4 เท่า, recall ~99%

float32 → binary (1 bit ต่อมิติ): เก็บแค่ 'บวก/ลบ' ของแต่ละมิติ
  → เล็กลง 32 เท่า, ค้นเร็วมาก (Hamming distance) แต่ recall ตกลง

→ กู้ recall ด้วย "rescoring": ค้นหยาบด้วย binary เอา candidate เยอะ ๆ (เช่น 100)
  แล้ว re-rank ด้วยเวกเตอร์ float เต็มเฉพาะ 100 ตัวนั้น → ได้ทั้งเร็วและแม่น`, cap: "binary quant + rescoring = เก็บเล็กลง 32 เท่า แต่ความแม่นเกือบเท่าเดิม — เทคนิคยอดนิยมระดับ billion-scale", lang: "txt" },
      { note: "ลองคำนวณเอง: 10M เอกสาร embedding 1536 มิติ float32 = 61 GB (ใส่ RAM ไม่ไหวบนเครื่องเล็ก). int8 = 15 GB, binary = 1.9 GB → เลือก quantization ตามว่า vector ต้องอยู่ใน RAM ไหม" },
      { qa: [
        { q: "ทำไมไม่ใช้มิติเยอะ ๆ ไปเลยให้แม่นสุด?", a: "มิติเยอะ = storage/RAM เปลือง + ค้นช้า + ค่า embed บางทีแพงกว่า. รุ่นใหม่ตัดมิติได้ (Matryoshka) → เลือกจุดสมดุล accuracy/cost ไม่ใช่ max เสมอ" },
        { q: "quantization ทำให้ค้นพลาดเยอะไหม?", a: "int8 แทบไม่ตก (~99% recall); binary ตกชัดกว่าแต่กู้คืนได้ด้วย rescoring (ค้นหยาบเอา candidate เยอะ แล้ว re-rank ด้วย float เต็ม)" },
        { q: "Matryoshka embedding คืออะไร?", a: "วิธีฝึกให้ 'มิติต้น ๆ สำคัญสุด' → ตัดเวกเตอร์ให้สั้นลง (เช่น 1536→256) แล้วยังใช้ได้ดี โดยไม่ต้อง embed ใหม่ — ลด storage และค้นเร็วขึ้น" },
      ]},

      { qa: [
        { q: "quantization ในบริบทของเวกเตอร์คืออะไร?", a: "ลดความละเอียดของแต่ละมิติ (เช่นจาก 32 บิตเหลือ 8 บิตหรือ 1 บิต) เพื่อให้เก็บเล็กลงและเทียบเร็วขึ้น — แลกกับความแม่นยำที่ลดลงเล็กน้อย" },
        { q: "ทำไมยังต้องมีขั้น rescoring?", a: "เพราะผลจากเวกเตอร์ที่ย่อแล้วอาจจัดลำดับคลาดเคลื่อน — ดึงผู้สมัครจำนวนมากกว่าที่ต้องการด้วยเวกเตอร์ย่อ แล้วจัดลำดับใหม่ด้วยเวกเตอร์เต็มเฉพาะกลุ่มนั้น" },
        { q: "ลดจำนวนมิติกับลดความละเอียดต่อมิติ ต่างกันยังไง?", a: "ลดมิติคือตัดข้อมูลบางส่วนทิ้งทั้งแกน (ซึ่งบางโมเดลออกแบบมาให้ตัดท้ายได้); ลดความละเอียดคือเก็บทุกแกนแต่หยาบลง. ทั้งสองใช้ร่วมกันได้" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
→ chunk ที่ 1 อาจมี "ข้อ 1" ครบ, chunk ที่ 2 อาจมี "ข้อ 2" ครบ
   → LLM ได้บริบทที่ถูกต้อง`, cap: "Recursive = หั่นตาม separator ตามลำดับความสำคัญ → รักษาโครงสร้างประโยค/ย่อหน้า", lang: "txt" },
      ...
      { qa: [
        { q: "ทำไมต้องหั่นเอกสารก่อนทำ embedding?", a: "เพราะเวกเตอร์เดียวแทนเอกสารทั้งฉบับได้แย่ — ใจความหลายเรื่องถูกเฉลี่ยรวมกันจนไม่ตรงกับคำถามไหนเลย. หั่นเป็นชิ้นที่พูดเรื่องเดียวทำให้แต่ละเวกเตอร์มีความหมายชัด" },
        { q: "ทำไมต้องให้ชิ้นซ้อนกัน (overlap)?", a: "เพื่อไม่ให้ใจความที่คร่อมรอยตัดหายไป — ประโยคที่ตอบคำถามอาจถูกตัดครึ่งพอดี. ซ้อนกันเล็กน้อยทำให้ยังมีชิ้นหนึ่งที่มีใจความครบ" },
        { q: "ขนาดชิ้นควรเป็นเท่าไหร่?", a: "ไม่มีค่าเดียวที่ใช้ได้ทุกงาน — ขึ้นกับลักษณะเอกสารและคำถาม. วิธีที่ถูกคือวัดด้วยชุดคำถามจริงของตัวเองแล้วเทียบผล ไม่ใช่ลอกค่าจากบทความ" },
      ]},
...
-- ถ้าจะทำจริงใช้ FULL OUTER JOIN แล้วเติม 0 ให้ฝั่งที่ขาด (หรือใช้ RRF)`, cap: "Postgres ทำ hybrid search ได้ใน DB เดียว pgvector + tsvector — แต่การรวมคะแนนต้องระวังสเกลกับ JOIN", lang: "txt" },
      ...
      { qa: [
        { q: "BM25 กับการค้นด้วยเวกเตอร์ต่างกันยังไง?", a: "BM25 จับคำที่ตรงกันจริง ๆ (ดีกับรหัสสินค้า ชื่อเฉพาะ ตัวย่อ); เวกเตอร์จับความหมายใกล้เคียงแม้ใช้คนละคำ. จุดแข็งของแต่ละตัวคือจุดอ่อนของอีกตัวพอดี" },
        { q: "hybrid search รวมผล 2 ฝั่งยังไง?", a: "ได้ 2 แบบ — รวมคะแนนโดยปรับสเกลให้เทียบกันได้ก่อน หรือรวมด้วยอันดับแบบ RRF ซึ่งไม่ต้องสนใจสเกลของคะแนนเลย. แบบหลังทำง่ายกว่าและทนกว่า" },
        { q: "ทำไมรวมคะแนนดิบตรง ๆ ไม่ได้?", a: "เพราะคนละหน่วย — คะแนนความคล้ายอยู่ในช่วงจำกัด ส่วนคะแนนของ BM25 ไม่มีขอบบนตายตัว. รวมตรง ๆ ฝั่งที่คะแนนใหญ่กว่าจะกลบอีกฝั่งเสมอ" },
      ]},
...
→ วัดแยก 2 ชั้น = รู้จุดที่ต้องแก้ทันที (ไม่ใช่เดา)`, cap: "เทียบ Recall (ค้น) กับ Faithfulness (ตอบ) → ชี้ได้ว่าปัญหาอยู่ขั้นไหน", lang: "txt" },
      ...
      { qa: [
        { q: "ทำไมต้องวัดขั้นค้นกับขั้นตอบแยกกัน?", a: "เพราะคำตอบผิดมีได้ 2 สาเหตุ — หาเอกสารไม่เจอ หรือหาเจอแล้วตอบผิด. วัดรวมกันจะไม่รู้ว่าควรไปแก้ตรงไหน" },
        { q: "golden set คืออะไร?", a: "ชุดคำถามพร้อมคำตอบและเอกสารที่ถูกต้องซึ่งเราตัดสินไว้เอง — เป็นฐานที่ทำให้เปรียบเทียบระหว่างเวอร์ชันได้อย่างมีความหมาย. ไม่มีชุดนี้ก็วัดการปรับปรุงไม่ได้เลย" },
        { q: "Recall@k บอกอะไร?", a: "ในผลลัพธ์ k อันดับแรก มีเอกสารที่ถูกต้องติดมากี่ส่วน — ถ้าค่านี้ต่ำ ปัญหาอยู่ที่ขั้นค้น ไม่ว่าจะปรับ prompt ของขั้นตอบยังไงก็ไม่ช่วย" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
# 10. Answer: "ส่งอีเมลสรุป AAPL ให้ทีมแล้ว ✓"`, cap: "create_react_agent = ReAct loop สำเร็จรูป — ไม่ต้องเขียน loop เอง", lang: "py" },
      ...
      { qa: [
        { q: "ReAct คืออะไร?", a: "วนสลับระหว่างการให้เหตุผล (Reason) กับการลงมือ (Act) แล้วอ่านผล (Observe) — แทนที่จะคิดจบทีเดียวแล้วตอบ. ทำให้แผนปรับตามสิ่งที่เจอจริงได้" },
        { q: "ทำไมดีกว่าให้เหตุผลอย่างเดียว?", a: "เพราะการให้เหตุผลล้วนอาศัยแต่สิ่งที่โมเดลจำได้ — พอได้ผลจากเครื่องมือจริงเข้ามา สมมติฐานที่ผิดจะถูกแก้ทันทีแทนที่จะถูกต่อยอดไปเรื่อย ๆ" },
        { q: "ข้อเสียของ ReAct คืออะไร?", a: "แต่ละรอบเพิ่ม context และค่าใช้จ่าย — และถ้าไม่มีเงื่อนไขหยุดที่ดี มันวนได้ไม่จบ. ต้องมีทั้งเพดานจำนวนรอบและตัวตรวจว่าไม่มีความคืบหน้า" },
      ]},
...
→ เริ่มจาก DAG → เพิ่ม cycle เมื่อจำเป็น → network เมื่อ scale`, lang: "txt" },
      ...
      { qa: [
        { q: "DAG กับ cyclic ต่างกันยังไงในเชิงการใช้งาน?", a: "DAG ไหลไปทางเดียวจบแน่นอน เหมาะกับงานที่ขั้นตอนรู้ล่วงหน้า; cyclic ย้อนกลับได้ เหมาะกับงานที่ต้องลองแล้วแก้ — แต่ต้องมีเงื่อนไขหยุดชัดเจน" },
        { q: "เลือกโครงสร้างยังไง?", a: "ถ้าเขียนลำดับขั้นออกมาได้ล่วงหน้าให้ใช้ DAG — ควบคุมง่ายและคาดเดาได้. ใช้แบบมีวงเฉพาะตอนที่จำนวนรอบขึ้นกับผลลัพธ์จริงเท่านั้น" },
        { q: "ทำไมไม่ใช้แบบยืดหยุ่นที่สุดไปเลย?", a: "เพราะยิ่งยืดหยุ่นยิ่งคาดเดายาก ทดสอบยาก และแพง — ความยืดหยุ่นที่ไม่ได้ใช้คือต้นทุนล้วน ๆ" },
      ]},
...
ชั้น 4 — Budget cap: นับ token/cost สะสม เกินเพดาน → ตัดจบทันที`, cap: "ทุก agent loop ต้องมีอย่างน้อย hard cap + budget cap — ไม่งั้นวันหนึ่งจะมีบิลพุ่ง", lang: "txt" },
      ...
      { qa: [
        { q: "agent วนไม่จบเพราะอะไร?", a: "มักเป็นเพราะเงื่อนไขจบวัดจาก 'ตัวเองคิดว่าเสร็จหรือยัง' ซึ่งโมเดลตอบว่ายังไม่เสร็จได้เรื่อย ๆ — ต้องมีเกณฑ์ที่ตรวจได้จากภายนอกด้วย เช่นเทสผ่านหรือไม่" },
        { q: "ควรมีกลไกหยุดกี่ชั้น?", a: "อย่างน้อยหลายชั้นที่ทำงานอิสระกัน — เพดานจำนวนรอบ, เพดานค่าใช้จ่าย, ตัวตรวจว่าไม่มีความคืบหน้า, และเงื่อนไขสำเร็จที่วัดจากภายนอก. ชั้นเดียวพลาดได้เสมอ" },
        { q: "เจอ error แล้วให้ลองใหม่ทุกครั้งดีไหม?", a: "ไม่ — ต้องแยกว่าเป็นความผิดพลาดชั่วคราว (ลองใหม่คุ้ม) หรือความผิดพลาดเชิงตรรกะ (ลองใหม่ก็ได้ผลเดิมและเผาเงินเปล่า)" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
# ค่าใช้จ่าย: ~$0 (regex ล้วน, ไม่ต้องเรียก LLM)`, cap: "Output guardrail = regex + rule-based → ถูก + เร็ว + ไม่ trust LLM output", lang: "py" },
      ...
      { qa: [
        { q: "prompt injection คืออะไร?", a: "การแทรกคำสั่งเข้าไปในข้อมูลที่ระบบจะอ่าน เพื่อให้โมเดลทำตามคำสั่งของผู้โจมตีแทนของเจ้าของระบบ — เพราะโมเดลไม่มีเส้นแบ่งโดยธรรมชาติระหว่าง 'คำสั่ง' กับ 'ข้อมูล'" },
        { q: "แบบตรงกับแบบอ้อมต่างกันยังไง?", a: "แบบตรงคือผู้ใช้พิมพ์เข้ามาเอง; แบบอ้อมคือคำสั่งซ่อนอยู่ในเนื้อหาที่ระบบไปดึงมา (หน้าเว็บ อีเมล เอกสาร) — แบบอ้อมอันตรายกว่าเพราะผู้ใช้เองก็ไม่รู้ตัว" },
        { q: "ป้องกันด้วย prompt อย่างเดียวพอไหม?", a: "ไม่พอ — ต้องกันที่ระดับสิทธิ์ด้วย. ให้เครื่องมือมีขอบเขตแคบที่สุด, ขั้นที่ทำอะไรย้อนไม่ได้ต้องมีคนอนุมัติ, และอย่าให้ข้อมูลจากภายนอกมีสิทธิ์เท่ากับคำสั่งของระบบ" },
      ]},
...
ถ้าใช้ TTL 1 ชั่วโมง: write เป็น 2 เท่า ($6.00) ไม่ใช่ 1.25 เท่า
     ...
     เลือก 1h เมื่อ traffic มาเป็นช่วง ๆ ห่างเกิน 5 นาที`, cap: "TTL 5 นาที คุ้มตั้งแต่รอบ 2 · TTL 1 ชั่วโมง ต้องรอถึงรอบ 3 เพราะ write แพงกว่า", lang: "txt" },
      ...
      { qa: [
        { q: "ลดค่าใช้จ่ายได้จากทางไหนบ้าง?", a: "เลือกรุ่นให้เหมาะกับงาน (งานง่ายไม่ต้องใช้รุ่นใหญ่), ใช้แคชกับส่วนนำหน้าที่ซ้ำ, ตัดข้อมูลที่ไม่เกี่ยวออกจาก context, และรวมงานที่ไม่เร่งไปทำแบบชุด" },
        { q: "ทำไมการจัดเส้นทางตามความยากถึงได้ผลมาก?", a: "เพราะสัดส่วนงานง่ายมักเป็นส่วนใหญ่ของทั้งหมด — ย้ายเฉพาะส่วนนั้นไปรุ่นที่ถูกกว่าก็ลดค่าใช้จ่ายรวมได้มากโดยไม่กระทบงานยาก" },
        { q: "วัดว่าประหยัดจริงยังไง?", a: "ดูจำนวนโทเคนจริงที่รายงานกลับมาในแต่ละคำขอ แยกเป็นอินพุต เอาต์พุต เขียนแคช และอ่านแคช — ไม่ใช่ประเมินจากจำนวนคำขอเพียงอย่างเดียว" },
      ]},
  ...
  + ง่าย scale ได้ ; - คะแนนสัมบูรณ์แกว่ง (judge ขี้เกรงใจ/เข้มไม่นิ่ง)

Pairwise (เทียบ 2 คำตอบ ไหนดีกว่า):
  judge อ่าน A vs B → ตอบ "A ดีกว่า" หรือ "B ดีกว่า"
  + แม่นกว่า (เทียบง่ายกว่าให้คะแนนเดี่ยว) ; - ต้องรันหลายคู่`, cap: "pairwise มักแม่นกว่า pointwise เพราะ 'เทียบ' ง่ายกว่า 'ให้คะแนนสัมบูรณ์'", lang: "txt" },
      { p: "**3 อคติที่ทำให้ judge ไม่น่าเชื่อ (ต้องแก้):**" },
      { code: String.raw`1. Position bias: pairwise มักลำเอียงเลือกตัว 'แรก' (หรือตัวหลัง) เสมอ
   แก้: รันสลับลำดับ (A,B) และ (B,A) แล้วเฉลี่ย — ถ้าไม่ตรงกัน = เสมอ

2. Verbosity bias: judge ชอบคำตอบ 'ยาว' ทั้งที่ไม่ได้ดีกว่า
   แก้: ใส่ rubric ว่า 'สั้นกระชับได้คะแนนเท่ากันถ้าครบ', ตัดความยาวออกจากเกณฑ์

3. Self-preference bias: judge ชอบสไตล์ของ 'โมเดลตัวเอง'
   แก้: ใช้ judge คนละ family จากตัวที่ generate`, cap: "ไม่แก้อคติ = คะแนนเอนเอียง เชื่อไม่ได้ — โดยเฉพาะ position bias ที่เจอบ่อยสุด", lang: "txt" },
      { p: "**การ calibrate — อย่าเพิ่งเชื่อ judge:** ก่อนใช้ judge วัดทั้งระบบ ให้เอา judge ไปทดกับ 'ชุดที่คนให้คะแนนไว้แล้ว' (~50 เคส) แล้วดูว่า judge เห็นด้วยกับคนกี่ % (agreement). ถ้า agreement สูง (เช่น >85%) ค่อยเชื่อ judge ไปวัดเคสที่เหลือเป็นพัน" },
      { code: String.raw`judge prompt ที่ดี (rubric ชัด + บังคับผล):
  "ให้คะแนนคำตอบเทียบ rubric ต่อไปนี้:
   - ถูกต้องตามข้อมูลที่ให้ (ไม่แต่งเติม)
   - ตอบตรงคำถาม
   คิดเหตุผลใน <thinking>...</thinking> ก่อน
   แล้วตอบบรรทัดสุดท้ายเป็น: VERDICT: correct | incorrect"
  → เก็บเฉพาะ VERDICT, ทิ้ง thinking ; temperature=0 ; ใช้โมเดลคนละตัว`, cap: "rubric เป็นรูปธรรม + บังคับ verdict เด็ดขาด + reason ก่อนตัดสิน = judge ที่ทำซ้ำได้", lang: "txt" },
      { note: "ลองทำเอง: เอา judge ไปให้คะแนน 20 คำตอบที่คุณรู้คำตอบอยู่แล้ว — นับว่ามันเห็นด้วยกี่ข้อ. ถ้าต่ำกว่า ~80% ปรับ rubric หรือเปลี่ยนโมเดล judge ก่อนเอาไปใช้จริง" },
      { qa: [
        { q: "pointwise กับ pairwise judge ต่างกันยังไง?", a: "pointwise ให้คะแนน 1 คำตอบเทียบ rubric (ง่าย scale ได้ แต่คะแนนแกว่ง); pairwise เทียบ 2 คำตอบว่าไหนดีกว่า (แม่นกว่าเพราะเทียบง่ายกว่า แต่ต้องรันหลายคู่)" },
        { q: "position bias คืออะไร แก้ยังไง?", a: "judge แบบ pairwise มักลำเอียงเลือกตัวแรก(หรือตัวหลัง)เสมอไม่ว่าเนื้อหา. แก้ด้วยรันสลับลำดับ (A,B) และ (B,A) แล้วเฉลี่ย — ไม่ตรงกัน = ตัดสินเป็นเสมอ" },
        { q: "เชื่อคะแนนจาก LLM judge ได้เลยไหม?", a: "ยัง — ต้อง calibrate ก่อน: เอา judge ทดกับชุดที่คนให้คะแนนไว้ (~50 เคส) ดู agreement กับคน; สูงพอ (เช่น >85%) ค่อยเชื่อไปวัดเคสจริงเป็นพัน" },
      ]},

      { qa: [
        { q: "LLM-as-judge คืออะไร ใช้ตอนไหน?", a: "ใช้โมเดลให้คะแนนผลลัพธ์ของโมเดล — เหมาะกับงานที่ไม่มีคำตอบถูกผิดตายตัว (เช่นคุณภาพการเขียน) ซึ่งเทียบสตริงตรง ๆ ไม่ได้" },
        { q: "การให้คะแนนแบบเทียบคู่ดีกว่าให้คะแนนเดี่ยวยังไง?", a: "เทียบ 2 ผลลัพธ์ว่าอันไหนดีกว่าทำได้สม่ำเสมอกว่าการให้เลข 1-10 ซึ่งเลื่อนไปมาระหว่างรอบ — โมเดลตัดสิน 'ดีกว่า' ได้เสถียรกว่าตัดสิน 'ดีแค่ไหน'" },
        { q: "อคติที่ต้องระวังมีอะไรบ้าง?", a: "ชอบคำตอบที่ยาวกว่า, ชอบตัวเลือกที่อยู่ตำแหน่งแรก, และชอบผลลัพธ์ที่รูปแบบเหมือนของตัวเอง. แก้ด้วยการสลับตำแหน่งแล้วตัดสินซ้ำ และสอบเทียบกับคะแนนของคนบางส่วน" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
10 รอบ ไม่ใช่ 10 เท่า แต่ ~55 เท่าของรอบเดียว!`, cap: "ต้นทุน input ของ loop โตแบบกำลังสอง (O(N²)) ตามจำนวนรอบ ถ้าไม่ตัด/สรุป context — นี่คือเหตุผลที่ 'cap จำนวนรอบ' อย่างเดียวยังไม่พอ ต้อง cap token ด้วย", lang: "txt" },
      ...
      { qa: [
        { q: "ทำไมค่าใช้จ่ายของ loop ถึงโตเร็วกว่าที่คิด?", a: "เพราะแต่ละรอบส่ง context สะสมทั้งหมดเข้าไปใหม่ — รอบที่ n จ่ายค่าอินพุตของทุกรอบก่อนหน้ารวมกัน ไม่ใช่แค่ของรอบนั้น" },
        { q: "ลดได้ยังไง?", a: "ย่อประวัติเป็นสรุปเมื่อยาวเกินเกณฑ์, เก็บเฉพาะผลลัพธ์ที่จำเป็นต่อรอบถัดไป, และใช้แคชกับส่วนนำหน้าที่ไม่เปลี่ยน" },
      ]},
...
ทั้งหมด = gate ผ่าน แต่ 'งานจริง' ไม่เสร็จ → false-pass`, cap: "loop ไม่ได้ 'ตั้งใจโกง' — มันแค่ทำตามแรงจูงใจที่ gate สร้าง. gate ที่ดีต้องปิดช่องเหล่านี้", lang: "txt" },
      ...
      { qa: [
        { q: "false-pass คืออะไร?", a: "loop 'ผ่าน' เงื่อนไขจบทั้งที่งานยังไม่เสร็จจริง — มักเกิดเมื่อเงื่อนไขวัดจากคำพูดของโมเดลเองแทนที่จะวัดจากผลที่ตรวจได้" },
        { q: "กันยังไง?", a: "ให้เงื่อนไขจบวัดจากภายนอก เช่นเทสที่รันจริง และอย่าให้ loop มีสิทธิ์แก้ตัวที่ใช้วัด — ไม่งั้นมันจะแก้ตัววัดแทนที่จะแก้งาน" },
      ]},
...
ReAct — แต่ละขั้นยึดผลจริงก่อนคิดต่อ:
  Reason → Act: read_file → Observe: "ไม่มี calc() ในไฟล์!"
   ...
   ✅ observation ดึงโมเดลกลับเข้าทาง + แก้แผนกลางคันได้`, cap: "ความต่างไม่ใช่ 'มีขั้นตอนเยอะกว่า' แต่คือ 'ทุกขั้นคิดบนข้อเท็จจริงที่เพิ่งเห็น' ไม่ใช่บนสมมติฐาน", lang: "txt" },
      ...
      { qa: [
        { q: "1 รอบของ loop ประกอบด้วยอะไร?", a: "ให้เหตุผลว่าจะทำอะไรต่อ → ลงมือ (เรียกเครื่องมือ) → อ่านผลที่ได้จริง แล้วเอาผลนั้นเข้าสู่รอบถัดไป" },
        { q: "ทำไมขั้น observe ถึงสำคัญที่สุด?", a: "เพราะเป็นจุดเดียวที่ข้อมูลจากโลกจริงเข้ามาแก้สมมติฐานผิด ๆ ได้ — ข้ามขั้นนี้แล้ว loop ก็แค่คิดวนอยู่กับตัวเอง" },
      ]},
...
สัญญาณว่า 'ติด' → abort ก่อน cap:
  ...
  → ทำ ① หรือ ③ ครบ → EXHAUSTED(stuck) → escalate พร้อม log`, cap: "convergence = failset หดลงเรื่อย ๆ; ถ้าไม่หด/โต/ซ้ำ/แกว่ง แปลว่าวนเปล่า ตัดจบเร็วดีกว่ารอจนชน cap", lang: "txt" },
      ...
      { qa: [
        { q: "รู้ได้ยังไงว่า loop ติด?", a: "เทียบสถานะระหว่างรอบ — ถ้าการกระทำและผลซ้ำเดิมหลายรอบติดกัน หรือสิ่งที่ใช้วัดความคืบหน้าไม่ขยับเลย ให้ถือว่าติดและหยุด" },
        { q: "ทำไมพึ่งเพดานจำนวนรอบอย่างเดียวไม่พอ?", a: "เพราะกว่าจะชนเพดานก็เผาไปหมดแล้ว — ตัวตรวจความคืบหน้าหยุดได้ตั้งแต่รอบที่ 3-4 แทนที่จะเป็นรอบที่ 50" },
      ]},
