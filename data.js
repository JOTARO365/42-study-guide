...
ผล = 1000 0001 = 129 = 'É' (ตัวอย่าง)`, cap: "เลื่อนซ้าย = เปิดที่ว่างขวาสุด, |1 เติมบิตใหม่ลงไป — MSB-first ทำให้ลำดับถูกต้องโดยอัตโนมัติ", lang: "txt" },
 ...
 main() printf ต่อ...               ← buffer มี "HelloWorld" ปนกัน → เพี้ยน!
      ...
      { p: "POSIX นิยามคำว่า **async-signal-safe** = ฟังก์ชันที่รับประกันว่าเรียกใน handler ได้ปลอดภัย มีแค่ **13 ฟังก์ชัน** ในรายการ (รวม `write`, `_exit`, `kill`, `sigaction`) — ที่เหลือห้ามเรียก" },
      { table: { head: ["ฟังก์ชัน", "ปลอดภัยใน handler?", "ทำไม"], rows: [
        ["`write()`", "✓ ปลอดภัย", "POSIX นิยามชัดว่าเป็น async-signal-safe"],
        ["`write`-based ft_printf", "✓ (ของเรา)", "因为我们สร้างบน write() ล้วน — ไม่มี internal buffer"],
...
ขั้นที่ 2 — ใส่ในรูปแบบ UTF-8 (3 bytes เพราะ U+0800..U+FFFF):
  ...
  เติม 12 บิตจาก U+0E01 = 0000 1110 0000 0001
...
UTF-8: 11110010 10011111 10001110 10001001
       = 0xF2    0x9F    0x8E    0x89
...
server รับ 4 bytes → write → terminal แสดง '🎉' ✓`, lang: "txt" },
      ...
      { links: [
        ...
        { label: "Async-signal-safe functions — man7", url: "https://man7.org/linux/man-pages/man7/signal-safety.7.html", note: "รายชื่อฟังก์ชัน 13 ตัวที่ปลอดภัยใน signal handler + ทำไมตัวอื่นไม่ปลอดภัย" },
