...
/* ===================== PUSH_SWAP ===================== */
{
  ...
  sections: {
    principle: [
      ...
      { h: "เกณฑ์คะแนน (ให้เป็นขั้น ไม่ใช่ผ่าน/ไม่ผ่าน)" },
      { table: { head: ["คะแนน", "100 ตัว", "500 ตัว"], rows: [
        ["5 (เต็ม)", "< 700 ops", "< 5500 ops"],
        ["4", "< 900", "< 7000"],
        ["3", "< 1100", "< 8500"],
        ["2", "< 1300", "< 10000"],
        ["1", "< 1500", "< 11500"],
      ]}},
      { note: "ยิ่ง ops น้อยยิ่งได้คะแนนสูง — ไม่มีเส้น 'ผ่าน/ไม่ผ่าน' ตายตัว. เล็งที่ขั้น 5 ไว้ก่อน แล้วขั้นอื่นเป็นตาข่ายรองรับ" },
      ...
      { p: "เป้าหมายจริงคือ 'ทำให้ถูก' (ง่าย) **บวกกับ** 'ใช้ ops ให้น้อยพอเข้าขั้นสูงสุด: <700 สำหรับ 100 ตัว, <5500 สำหรับ 500 ตัว' (ยาก) — ตรงนี้แหละที่ต้องเลือก algorithm ให้ดี" },
      { h: "เทียบทางเลือก algorithm (ทำไมถึงเลือก Turkish sort)" },
      { table: { head: ["วิธี", "แนวคิด", "ops ~100 ตัว", "สรุป"], rows: [
        ["Selection (ดัน min ทีละตัว)", "หา min → หมุนขึ้น top → pb วนไป แล้ว pa กลับ", "~5000", "เขียนง่ายสุด แต่โตแบบ n² — เกินเกณฑ์เต็มหลายเท่า (ดูได้จากปุ่มเดโม)"],
...
ตัวอย่าง: f(n) = 3n² + 5n + 2  เป็น O(n²)
  ...
  (พจน์ 5n+2 ถูกกลืนโดย n² ที่โตเร็วกว่า)`, cap: "Big-O มองเฉพาะ 'พจน์ที่โตเร็วที่สุด' และไม่สนค่าคงที่ — เพราะเมื่อ n ใหญ่ พจน์นั้นครอบงำทั้งหมด", lang: "txt" },
      ...
      { qa: [
        { q: "Big-O นิยามอย่างเป็นทางการว่ายังไง?", a: "f(n) = O(g(n)) ก็ต่อเมื่อมีค่าคงที่ c > 0 และ n₀ ที่ทำให้ 0 ≤ f(n) ≤ c·g(n) สำหรับทุก n ≥ n₀ — คือ g เป็นขอบบนของ f เมื่อ n ใหญ่พอ โดยไม่สนใจค่าคงที่" },
        { q: "O, Ω, Θ ต่างกันยังไง?", a: "`O` คือขอบบน (ไม่แย่กว่านี้) · `Ω` คือขอบล่าง (อย่างน้อยเท่านี้) · `Θ` คือทั้งสองด้านพร้อมกัน (โตเท่ากันจริง ๆ)" },
        { q: "ทำไมตัดค่าคงที่กับพจน์เล็กทิ้งได้?", a: "เพราะนิยามมี c ให้คูณอยู่แล้วและสนใจแค่ n ที่ใหญ่พอ — `3n² + 100n` กับ `n²` โตในอัตราเดียวกันเมื่อ n โตขึ้นเรื่อย ๆ" },
      ]},
...
ดังนั้นกรณีแย่สุดต้องเทียบอย่างน้อย ~n·log₂n ครั้ง`, cap: "ความสูงของต้นไม้ = จำนวนคำถามในเส้นทางที่ยาวสุด = จำนวนการเทียบกรณีแย่สุด", lang: "txt" },
  ...
  100! ≈ 9.33 × 10¹⁵⁷
  log₂(100!) ≈ 524.8  → ต้อง "เทียบ" อย่างน้อย ~525 ครั้ง
  n·log₂n = 100 × 6.64 ≈ 664   (ค่าประมาณของตัวเดียวกัน)`, cap: "ความสูงของต้นไม้ = จำนวนคำถามที่ต้องถามกรณีแย่สุด", lang: "txt" },
      { note: "⚠️ ระวังอย่าเอา 2 หน่วยนี้มาปนกัน: ~525 คือขอบล่างของ **จำนวนการเปรียบเทียบ** ส่วนที่ push_swap ให้คะแนนคือ **จำนวน operation ที่พิมพ์** (sa/ra/pb/...) ซึ่งเป็นคนละหน่วย. ทฤษฎีบทนี้จึงไม่ได้พิสูจน์ว่า 'ops ต้อง ≥ 525' — มันแค่บอกว่างานเรียงมีความยากขั้นต่ำระดับ n log n และเราใช้ n·log₂n เป็น 'ไม้บรรทัด' เทียบว่า algorithm ของเราเข้าใกล้ระดับนั้นแค่ไหน" },
      { note: "ด้วยไม้บรรทัดนี้: selection sort โต ~n²/2 (n=100 → ~5000 ops) ห่างระดับ n log n มาก ส่วน Turkish sort (~580 ops) อยู่ในระดับเดียวกับ n·log₂n = 664 เลย" },

      { qa: [
        { q: "ทำไม comparison sort ต้องใช้อย่างน้อย ~n·log₂n การเปรียบเทียบ?", a: "การเทียบแต่ละครั้งให้คำตอบได้ 2 ทาง จึงเป็นต้นไม้ binary — ต้นไม้สูง h มีใบได้มากสุด 2^h ใบ แต่ต้องแยกลำดับที่เป็นไปได้ n! แบบ ดังนั้น 2^h ≥ n! → h ≥ log₂(n!) ≈ n·log₂n" },
        { q: "ค่า ~525 ที่ n = 100 คือขอบล่างของอะไร?", a: "ของ **จำนวนการเปรียบเทียบ** ไม่ใช่จำนวน operation ที่ push_swap นับ — 2 หน่วยนี้คนละอย่างกัน เอามาเทียบตรง ๆ ไม่ได้" },
        { q: "push_swap หนีขอบล่างนี้ได้ไหม?", a: "หนีขอบล่างของ *การเปรียบเทียบ* ได้ถ้าใช้ radix sort (ซึ่งไม่ตัดสินด้วยการเทียบ) — แต่คะแนนวัดที่จำนวน operation ซึ่งเป็นคนละเรื่อง. n·log₂n จึงใช้เป็น 'ไม้บรรทัด' ของระดับความยากเท่านั้น" },
      ]},
...
วิธีกัน (ในโค้ด):  สะสมเลขใน long (64-bit, ช่วงใหญ่กว่ามาก)
   แล้วเทียบขอบ int เอง ก่อนค่าจะมีโอกาส overflow`, lang: "txt" },
      ...
      { qa: [
        { q: "ทำไมช่วงของ `int` ถึงไม่สมมาตร?", a: "two's complement ให้เลข 0 อยู่ฝั่งบวก จึงเหลือฝั่งบวกน้อยกว่าฝั่งลบ 1 ค่า — ได้ -2147483648 ถึง 2147483647" },
        { q: "ตรวจ overflow ตอนแปลง argv ยังไง?", a: "สะสมค่าใน `long` แล้วเช็คว่าเกิน `INT_MAX` หรือน้อยกว่า `INT_MIN` หรือยัง — เช็คบน `int` เองไม่ได้เพราะมันวนไปแล้วก่อนที่เราจะทันเห็น" },
        { q: "`INT_MAX + 1` ได้อะไร?", a: "`INT_MIN` — บิตบนสุดพลิกจาก 0 เป็น 1 ซึ่งในระบบนี้แปลว่าค่าติดลบที่น้อยที่สุด. ไม่มี error ไม่มีคำเตือน" },
      ]},
...
(เพราะส่วนที่ซ้อนกันได้ถูกยุบด้วย rr/rrr)`, cap: "หัวใจที่ทำให้ op count ต่ำลงจนผ่านเกณฑ์", lang: "txt" },
      ...
      { qa: [
        { q: "cost ของการย้าย 1 ตัวคำนวณจากอะไร?", a: "จำนวนการหมุนที่ต้องใช้เพื่อเอาตัวนั้นขึ้นบน B (cb) บวกกับจำนวนที่ต้องหมุน A ให้ตำแหน่งเป้าขึ้นบน (ca) — แต่ละฝั่งเลือกทิศที่สั้นกว่าโดยเทียบตำแหน่งกับ size/2" },
        { q: "ทำไม cost ถึงเป็นค่าติดลบได้?", a: "เครื่องหมายเก็บ *ทิศทาง* ไว้ในตัวเลขเดียวกัน — บวกคือหมุนไปหน้า (`ra`/`rb`) ลบคือหมุนย้อน (`rra`/`rrb`). ทำให้ไม่ต้องมีตัวแปรทิศแยก" },
        { q: "`rr` ประหยัดตอนไหน?", a: "เมื่อ ca กับ cb เป็นบวกทั้งคู่ (หรือลบทั้งคู่) — หมุนพร้อมกันได้ ops ที่ใช้จึงเป็น `max(|ca|, |cb|)` แทนที่จะเป็น `|ca| + |cb|`" },
      ]},
...
แบบ rank (สมมติ n=5, 42 rank=1):
  ...
  → คำนวณด้วยเลข 0..4 ล้วน ไม่มีทาง overflow`, cap: "rank เปลี่ยนปัญหา 'เทียบค่าที่กระจายทั่ว 32 bit' เป็น 'เทียบเลข 0..n-1'", lang: "txt" },
      { note: "normalization เป็น O(n²) (เทียบทุกคู่) แต่ทำแค่ครั้งเดียวตอนต้น — คุ้มเพราะจากนั้น algorithm ทั้งหมดทำงานบน rank ล้วน ๆ ที่ O(1) ต่อการเทียบ" },

      { qa: [
        { q: "normalize คืออะไร ทำไมต้องทำ?", a: "แปลงค่าจริงเป็น rank 0..n-1 (ลำดับที่ของมันเมื่อเรียงแล้ว) — ทำให้เทียบค่าได้โดยไม่ต้องกลัว overflow และเทียบ index ตรง ๆ ได้เลยแทนที่จะเทียบค่าที่ห่างกันเท่าไหร่ก็ได้" },
        { q: "พิสูจน์ยังไงว่า normalize ไม่ทำให้ลำดับเพี้ยน?", a: "rank(x) = จำนวนสมาชิกที่น้อยกว่า x — ถ้า x < y จะมีสมาชิกที่น้อยกว่า y มากกว่าที่น้อยกว่า x อย่างน้อย 1 ตัว (คือ x เอง) ดังนั้น rank(x) < rank(y) เสมอ. ลำดับจึงคงเดิมทุกคู่" },
        { q: "normalize เป็น O(n²) ไม่ช้าไปหรือ?", a: "ทำครั้งเดียวตอนเริ่ม และคะแนนวัดที่จำนวน operation ของ stack ไม่ได้วัดเวลา — n² ที่ n=500 คือ 250,000 ครั้งซึ่งเสร็จในเสี้ยววินาที" },
      ]},
...
→ เหลือ 3 ตัวใน A → sort_3 ใช้ ≤ 2 ops (hardcode)
...
ผลจริง (วัดจากเดโม) เทียบกับไม้บรรทัด n·log₂n:
  n=100  → ~580 ops   (n·log₂n = 664)  → 0.87 เท่า  ต่ำกว่าไม้บรรทัด
  n=500  → ~5233 ops  (n·log₂n = 4482) → 1.17 เท่า  สูงกว่าไม้บรรทัด

  → อัตราส่วนไต่ขึ้นเมื่อ n โต (0.87 → 1.17) = greedy เสียเปรียบขึ้นเรื่อย ๆ
    แต่ยังอยู่ในระดับ n log n ไม่ใช่ n² — จึงผ่านเกณฑ์ทั้งสองขนาด`, cap: "greedy ไม่ optimal เชิงทฤษฎี แต่ในทางปฏิบัติให้ผล 'ดีพอ' สำหรับโจทย์นี้", lang: "txt" },
...
Turkish (greedy):
  ...
  n=100 → ~580 ops (ต่ำกว่าเกณฑ์ชัดเจน)`, cap: "Turkish sort ชนะเพราะ constant factor ต่ำ — rr/rrr + cost-based selection ทำให้แต่ละรอบคุ้มสุด", lang: "txt" },
      { note: "เหตุผลเชิงลึก: greedy ใช้ได้ดีที่นี่เพราะ 'cost function มีโครงสร้างเชิงเส้นพอ' — ค่าใช้จ่ายในการแทรกไม่ได้เพิ่มขึ้นแบบทวีคูณเมื่อ n โต จึงเลือกตัวถูกสุดได้ผลรวมที่ดี" },

      { qa: [
        { q: "greedy ไม่การันตีผลดีที่สุด แล้วทำไมใช้ได้ที่นี่?", a: "เพราะ cost function มีโครงสร้างที่การเลือกถูกในแต่ละก้าวไม่ทำให้ก้าวถัดไปแย่ลงมาก — การเอาตัวที่ย้ายถูกที่สุดออกไปก่อนทำให้กองที่เหลือเล็กลงและ cost เฉลี่ยลดตาม. ผลจริงจึงโตประมาณ n·log n" },
        { q: "ต่างจาก selection sort ยังไง?", a: "selection sort เลือกตัวที่ *เล็กที่สุด* เสมอโดยไม่สนว่าย้ายแพงแค่ไหน — ได้ ~n²/2 ops. Turkish sort เลือกตัวที่ *ย้ายถูกที่สุด* ซึ่งใช้ประโยชน์จากลำดับที่บังเอิญดีอยู่แล้วในกอง" },
        { q: "worst case ของ greedy แย่แค่ไหน?", a: "ทางทฤษฎีแย่ได้ถึง n² แต่ต้องเป็นอินพุตที่จงใจสร้าง — อินพุตสุ่มซึ่งเป็นสิ่งที่กรรมการใช้แทบไม่เข้าใกล้กรณีนั้นเลย" },
      ]},
      { h: "🔬 เจาะลึก G: วิเคราะห์เกณฑ์คะแนน — ทำไม <700 และ ≤5500" },
      { p: "หลายคนสงสัยว่าเกณฑ์ `< 700 ops (100 ตัว)` และ `< 5500 ops (500 ตัว)` ตั้งมาจากไหน. คำตอบไม่ใช่ 'มีทฤษฎีบทบอกว่าต้องเป็นเลขนี้' — แต่มาจากการเทียบกับ **ระดับ n log n** ซึ่งเป็นความยากขั้นต่ำของงานเรียง แล้วเผื่อช่องให้ algorithm ที่ดีจริงทำได้" },
      { code: String.raw`ไม้บรรทัด n·log₂n (ระดับความยากของงานเรียง):

n = 100:  100 × 6.64 = 664    → เกณฑ์เต็ม 700  = 1.05 เท่าของไม้บรรทัด
n = 500:  500 × 8.97 = 4482   → เกณฑ์เต็ม 5500 = 1.23 เท่า

→ เกณฑ์ถูกวางไว้ "แค่เหนือ" ระดับ n log n เล็กน้อย
  แปลว่า: ถ้า algorithm ของเราโตแบบ n log n จะผ่าน
          ถ้าโตแบบ n² จะไม่มีทางผ่าน`, cap: "เกณฑ์เต็มถูกออกแบบให้ 'ยากแต่ทำได้ถ้าเลือก algorithm ถูก' — มันคัด n log n ออกจาก n²", lang: "txt" },
      { code: String.raw`วิเคราะห์ว่า algorithm แต่ละตัวผ่านเกณฑ์ไหม:

n=100, เกณฑ์เต็ม < 700:
  Selection:  ~5000       ← เกินหลายเท่า ไม่มีทาง
  Radix:      ~700-1100   ← ก้ำกึ่ง (บางรอบผ่าน บางรอบไม่)
  Turkish:    ~580        ← ผ่านชัดเจนทุกรอบ

n=500, เกณฑ์เต็ม < 5500:
  Selection:  ~125000     ← เกินหลายสิบเท่า
  ...
  Turkish:    ~5233       ← ผ่านชัดเจน`, cap: "Turkish sort เป็น algorithm เดียวที่ผ่านเกณฑ์เต็มได้ทุกรอบอย่างมั่นคง", lang: "txt" },
      { code: String.raw`ทำไมเกณฑ์เต็มถึงตั้งชิดระดับ n log n ขนาดนั้น:

เป้าหมายคือบังคับให้เลือก algorithm ที่ "ดีจริง" ไม่ใช่ "พอทำงานได้":
  - ถ้าตั้งที่ 2000 → radix ก็ผ่านสบาย → ไม่ต้องคิดเรื่อง cost เลย
  - ถ้าตั้งที่ 550  → แทบไม่มีใครทำได้ → กลายเป็นด่านสุ่ม
  - 700 = จุดที่ "เข้าใจปัญหาแล้วออกแบบ cost เป็น" ถึงจะผ่าน

ส่วนขั้นคะแนนที่รองลงมา (900 / 1100 / ...) มีไว้ให้คนที่เลือก
radix หรือ greedy ที่ยังจูนไม่สุด ยังได้คะแนนตามที่ทำได้จริง`, cap: "เกณฑ์คือ 'ยาขมที่พอดี' — บังคับให้เรียน algorithm จริง แต่ไม่โหดจนท้อ", lang: "txt" },
      { note: "วิธีเช็คว่าได้คะแนนขั้นไหน: รัน 5-10 รอบด้วย input สุ่ม 100/500 ตัว แล้วดูค่า **max** ops (ไม่ใช่ค่าเฉลี่ย) — กรรมการสุ่มชุดเดียว เราจึงต้องรอดกรณีแย่สุด" },

      { qa: [
        { q: "เกณฑ์ 700 กับ 5500 มาจากไหน?", a: "ตั้งชิดระดับ n·log₂n (100 → 664, 500 → 4483) เพื่อบังคับให้ต้องใช้ algorithm ระดับ n log n จริง ๆ — วิธีที่โตแบบ n² ผ่านไม่ได้แน่นอน" },
        { q: "ได้ 690 ครั้งหนึ่งแปลว่าผ่านไหม?", a: "ยังสรุปไม่ได้ — ต้องรัน 5-10 รอบด้วยอินพุตสุ่มแล้วดูค่าที่แย่ที่สุด เพราะกรรมการก็สุ่มเหมือนกันและจะไม่ได้ชุดเดียวกับเรา" },
        { q: "ถ้าทำได้ 800 ops ที่ 100 ตัว ได้กี่คะแนน?", a: "ระดับ 4 (< 900) — ยังผ่านแต่ไม่เต็ม. ตารางคะแนนไล่ตั้งแต่ < 700 (เต็ม 5) ลงไปจนถึง < 1500 (1 คะแนน)" },
      ]},
      ...
      { links: [
        ...
        { label: "Greedy algorithm — Wikipedia", url: "https://en.wikipedia.org/wiki/Greedy_algorithm", note: "เมื่อไหร่ greedy ได้ผลดี vs เมื่อไหร่ไม่ได้ (matroid theory)" },
...
pa(ps);                      // ดันเข้า A`, lang: "c" },
    ...
    tricks: [
      { h: "ทริค 1: รวมการหมุนด้วย rr / rrr" },
      { p: "เมื่อ A กับ B ต้องหมุนทิศเดียวกัน ใช้ `rr` แทน `ra` + `rb` — จำนวน ops ของการย้ายครั้งนั้นกลายเป็น `max(|ca|, |cb|)` แทน `|ca| + |cb|`. **นี่คือตัวช่วยหลักที่ทำให้ผ่านเกณฑ์ 700/5500**" },
      { h: "ทริค 2: เก็บทิศไว้ในเครื่องหมายของ cost" },
      { p: "`+3` แปลว่า `ra` 3 ครั้ง · `-2` แปลว่า `rra` 2 ครั้ง — ทำให้ไม่ต้องมีตัวแปรทิศแยก และการเช็คว่า 'ทิศเดียวกันไหม' กลายเป็นการเทียบเครื่องหมายสองตัว" },
      { h: "ทริค 3: normalize เป็น rank ตั้งแต่ต้น" },
      { p: "หลังแปลงเป็น 0..n-1 แล้ว algorithm ทั้งหมดทำงานกับ `idx` — ไม่ต้องกังวล overflow อีกเลย และเทียบตำแหน่งกับ `idx` ตรง ๆ ได้" },
      { h: "ทริค 4: A เป็นวงกลมเรียง — จบด้วยการหมุนครั้งเดียว" },
      { p: "ระหว่างแทรก A ไม่ต้องเรียงจากเล็กไปใหญ่จากบนลงล่าง — ขอแค่ **เรียงเมื่อมองเป็นวงกลม** แล้วหมุนตัวเล็กสุดขึ้นบนทีเดียวตอนจบ. เงื่อนไขนี้หลวมกว่ามาก จึงแทรกได้ถูกกว่า" },
      { h: "ทริค 5: silent flag ใช้โค้ดชุดเดียวกับ checker" },
      { p: "`ps->silent` ทำให้ฟังก์ชัน op ชุดเดียวใช้ได้ทั้งตอนสร้างคำสั่ง (พิมพ์) และตอนเล่นคำสั่งใน checker (ไม่พิมพ์) — ไม่ต้องเขียน op ซ้ำ 2 ชุดที่จะหลุดไม่ตรงกันทีหลัง" },
      { h: "ทริค 6: bring_to_top เลือกทิศที่สั้นกว่าเสมอ" },
      { p: "เทียบ `pos` กับ `size / 2` — อยู่ครึ่งบนหมุนไปหน้า อยู่ครึ่งล่างหมุนย้อน. บรรทัดเดียวนี้ตัด ops ไปได้เกือบครึ่งเมื่อเทียบกับการหมุนไปหน้าอย่างเดียว" },
      { h: "ทริค 7: ตรวจ overflow ด้วย `long` แยกขอบบวก/ลบ" },
      { p: "สะสมค่าใน `long` แล้วเทียบกับ `INT_MAX` / `INT_MIN` — เช็คบน `int` เองไม่ได้เพราะมันวนไปก่อนที่เราจะทันเห็น" },
      { h: "ทริค 8: วัดด้วยค่าที่แย่ที่สุดจากหลายรอบ ไม่ใช่รอบเดียว" },
      { p: "อินพุตสุ่มทำให้ผลแกว่งได้หลายสิบ ops — รัน 5-10 รอบแล้วดูค่าสูงสุด เพราะกรรมการก็สุ่มเหมือนกันและจะไม่ได้ชุดเดียวกับเรา" },
    ],
    eval: [
      { qa: [
        ...
        { q: "stack กับ array ต่างกันตรงไหนในเชิงข้อจำกัด?", a: "stack เข้าถึงได้เฉพาะยอด (LIFO) — เอาตัวที่ 5 ออกมาตรง ๆ ไม่ได้ ต้องหมุนหรือดันออกไปก่อน. นี่คือสิ่งที่ทำให้โจทย์ยาก ถ้าเข้าถึงทุก index ได้อิสระก็แค่เรียงธรรมดา" },
        { q: "ทำไมเรียง 3 ตัวถึง hardcode ได้?", a: "3 ตัวมีลำดับที่เป็นไปได้แค่ 3! = 6 แบบ — เขียน if ครอบทั้ง 6 กรณีได้และจบใน ≤ 2 ops ทุกกรณี ซึ่งไม่มี algorithm ทั่วไปตัวไหนทำได้ดีกว่า" },
        { q: "sort_3 ใช้กี่ ops อย่างมาก?", a: "2 ops — ทุกกรณีใน 6 แบบจบได้ภายใน 2 การกระทำ" },
        { q: "ทำไมต้องดันลง B จนเหลือ 3 ตัว ไม่ใช่เหลือ 2 หรือ 5?", a: "3 คือจำนวนน้อยสุดที่ยัง hardcode ได้อย่างคุ้ม — เหลือ 2 ก็ต้องดันเพิ่มโดยไม่ได้อะไร ส่วนเหลือมากกว่านั้น sort ที่เหลือใน A จะแพงกว่าการดันลงไปแล้วดึงกลับ" },
        { q: "อธิบายขั้นตอนของ Turkish sort ทั้งหมด", a: "1) normalize เป็น rank · 2) ดันลง B จนเหลือ 3 ตัวใน A · 3) sort_3 · 4) วนหาตัวใน B ที่ cost รวมถูกที่สุดแล้วดึงกลับเข้า A ทีละตัว (ใช้ rr/rrr ช่วยเมื่อทิศตรงกัน) · 5) หมุนตัวที่เล็กสุดขึ้นบน" },
        { q: "หา 'ตำแหน่งเป้าใน A' ของตัวใน B ยังไง?", a: "หาตัวใน A ที่เล็กที่สุดในบรรดาตัวที่ *ใหญ่กว่า* ค่านั้น — ถ้าไม่มีเลย (ค่านั้นใหญ่สุดหรือเล็กสุด) เป้าคือตำแหน่งของตัวที่เล็กที่สุดใน A" },
        { q: "ทำไมท้ายสุดต้องหมุนตัวเล็กสุดขึ้นบน?", a: "เพราะระหว่างแทรก A ถูกทำให้เรียงแบบวงกลม — ตัวเล็กสุดอาจไม่ได้อยู่บนสุด. หมุนครั้งเดียวก็จบ และถูกกว่าการบังคับให้เรียงจากบนตลอดทาง" },
        { q: "ทดสอบว่าผ่านเกณฑ์จริงยังไง?", a: "รันด้วยอินพุตสุ่ม 100 และ 500 ตัวอย่างละ 5–10 รอบ แล้วดูค่าที่แย่ที่สุด ไม่ใช่ค่าที่ดีที่สุด — พร้อมส่งผ่าน `checker_linux` ทุกรอบเพื่อยืนยันว่าเรียงถูกด้วย" },
        { q: "ถ้า argv เป็น string เดียวที่มีหลายเลข เช่น `\"3 2 1\"` ต้องรับไหม?", a: "ต้อง — subject อนุญาตรูปแบบนั้น. แยกด้วย `ft_split` ก่อนแล้วค่อยตรวจทีละตัว และ string ว่างหรือมีแต่ช่องว่างต้องเป็น Error" },
      ]},
...
parent ไม่ exec → คงตัวเป็น "ผู้ควบคุม" รอ wait ลูกทั้งคู่`, cap: "ถ้า parent ไป exec เองจะหายไป ไม่เหลือใคร wait → เก็บกวาดไม่ได้", lang: "txt" },
      ...
      { qa: [
        { q: "`fork()` คืนค่าอะไรบ้าง แต่ละค่าหมายถึงอะไร?", a: "คืน 2 ครั้งจาก 2 process — ใน parent คืน PID ของลูก (มากกว่า 0) ใน child คืน 0 และคืน -1 เมื่อสร้างไม่สำเร็จ. โค้ดบรรทัดเดียวกันจึงเดินคนละทางใน 2 process" },
        { q: "หลัง `fork` ลูกได้อะไรติดไปบ้าง?", a: "สำเนาของทุกอย่าง — ตัวแปร, หน่วยความจำ, และ **ตาราง fd ทั้งตาราง**. อย่างหลังคือต้นเหตุของ pipeline ที่ค้าง เพราะ fd ที่ลืมปิดถูกก๊อปตามไปทุก process" },
        { q: "ทำไมต้องเช็คค่าคืนของ `fork` ทุกครั้ง?", a: "เพราะ -1 แปลว่าไม่มี process ใหม่เกิดขึ้น — ถ้าเดินต่อโดยไม่เช็ค โค้ดของ 'ลูก' จะทำงานใน parent และทำลาย fd ของมันเอง" },
      ]},
...
หลังจากนี้ exec(ls): ls เขียน stdout(fd1) = เข้า pipe โดยไม่รู้ตัว`, cap: "exec รักษาตาราง fd ไว้ (ไม่รีเซ็ต) → cmd จึงสืบทอดการ redirect นี้ไป", lang: "txt" },
      ...
      { qa: [
        { q: "`dup2(oldfd, newfd)` ทำอะไร?", a: "ปิด `newfd` (ถ้าเปิดอยู่) แล้วทำให้มันชี้ไปยังที่เดียวกับ `oldfd` — หลังจากนั้น fd ทั้งสองใช้แทนกันได้. `dup2(fd, 0)` จึงแปลว่า 'ตั้งแต่นี้ไป stdin คือไฟล์นี้'" },
        { q: "ทำไมต้อง `close(oldfd)` หลัง `dup2`?", a: "เพราะหลัง dup2 มี fd 2 ตัวชี้ที่เดียวกัน — ตัวเดิมไม่ได้ใช้แล้วแต่ยังนับเป็นผู้ถือปลายท่ออยู่ ซึ่งทำให้ฝั่งอ่านไม่มีวันเห็น EOF" },
        { q: "`dup2` ต่างจาก `dup` ยังไง?", a: "`dup` ให้ fd ว่างตัวเล็กที่สุดที่ระบบเลือกเอง; `dup2` ให้เราระบุหมายเลขปลายทางเอง ซึ่งจำเป็นเพราะเราต้องการหมายเลข 0 และ 1 โดยเฉพาะ" },
      ]},
  ...
  → wc (ใน child2) อ่าน stdin รอ EOF ตลอดกาล → ค้าง`, cap: "วิธีแก้: parent ต้อง close ปลาย pipe ทั้งสองข้างทันทีหลัง fork ครบ", lang: "txt" },
      ...
      { qa: [
        { q: "ทำไม `wc -l` ถึงค้างไม่ยอมจบ?", a: "`read` บน pipe จะคืน 0 (EOF) ก็ต่อเมื่อ **ปลายเขียนถูกปิดครบทุกสำเนาในทุก process**. เหลือค้างที่ไหนสักที่เดียว — รวมทั้งใน parent — ฝั่งอ่านก็รอตลอดไป" },
        { q: "ใครบ้างที่ต้องปิดปลายท่อ?", a: "parent ต้องปิดทั้ง 2 ปลายหลัง fork ครบแล้ว · ลูกแต่ละตัวต้องปิดปลายที่ตัวเองไม่ใช้ **และปิดปลายที่ dup2 ไปแล้วด้วย**" },
        { q: "ไล่หาสาเหตุตอนค้างยังไง?", a: "ตรวจทุก process ว่าเหลือปลายเขียนเปิดอยู่ไหม — `ls -l /proc/<pid>/fd` บอกได้ตรง ๆ ว่า fd ไหนยังเปิดและชี้ที่ไหน" },
      ]},
...
execve(ls): ทุกส่วนข้างบนถูก "ล้างทิ้ง" แล้วแทนด้วยของ ls
  ...
  → PID เดิม แต่ "ตัวตน" เป็นโปรแกรมใหม่หมด`, cap: "ถ้า execve สำเร็จจะไม่ return; โค้ดบรรทัดถัดไปคือ error path เสมอ", lang: "txt" },
      { qa: [
        { q: "`execve` ทำอะไรกับ process ที่เรียกมัน?", a: "แทนที่ภาพหน่วยความจำทั้งหมด (code, data, heap, stack) ด้วยโปรแกรมใหม่ — PID เดิม, fd ที่เปิดอยู่ยังอยู่ครบ, แต่ตัวแปรและโค้ดเดิมหายหมด" },
        { q: "ทำไมโค้ดหลัง `execve` ถึงเป็น error path เสมอ?", a: "เพราะถ้าสำเร็จมันไม่คืนค่า — โปรแกรมเดิมไม่มีอยู่แล้ว. บรรทัดถัดไปทำงานได้ก็ต่อเมื่อ exec ล้มเหลว" },
        { q: "ทำไม `execve` ต้องส่ง `envp`?", a: "เพราะโปรแกรมใหม่ต้องการ environment ของตัวเอง — ที่สำคัญคือ `PATH` ที่คำสั่งลูก ๆ ของมันจะใช้ต่อ. ส่ง NULL ไปแล้วหลายคำสั่งจะทำงานผิดปกติ" },
        { q: "`command not found` ควร exit ด้วยรหัสอะไร?", a: "127 ตามมาตรฐานของ shell — และ 126 เมื่อเจอไฟล์แต่รันไม่ได้ (ไม่มีสิทธิ์ execute)" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
    ...
    foundations: [
      ...
      { p: "`char *` = pointer ไป string (จริง ๆ คือที่อยู่ของตัวอักษรตัวแรก). `char **` คือ **pointer ไป array ของ string** = 'รายการของหลาย ๆ string' เช่น argv, envp และผลลัพธ์ของ ft_split ล้วนเป็น `char **`" },
...
{
    if (access(cmd, X_OK) == 0)      // norm ห้าม ?: → ใช้ if
...
run_child(prev, fds[1], cmds[n-1], envp);  // cmd สุดท้าย → outfile`, cap: "ขยายจาก 2 เป็น N commands ด้วยการเลื่อน pipe", lang: "c" },
    ...
    tricks: [
      { h: "ทริค 1: ปิด fd ให้ครบ — และปิดตัวที่ dup2 ไปแล้วด้วย" },
      { p: "fd ที่ลืมปิดคือสาเหตุอันดับ 1 ที่ pipeline ค้าง. **หลัง `dup2` แล้ว fd ตัวเดิมยังนับเป็นผู้ถือปลายท่ออยู่** — ต้องปิดมันด้วยเสมอ ไม่ใช่ปิดแค่ปลายที่ไม่ได้ใช้" },
      { h: "ทริค 2: infile หายไม่ใช่เรื่องถึงตาย" },
      { p: "shell จริงพิมพ์ error แล้วยังสร้าง outfile ต่อ — ใช้ `perror` แล้วเดินต่อ อย่า `exit` ทันที. ทำเหมือน shell แล้วกรรมการเทียบพฤติกรรมกับของจริงได้ตรง" },
      { h: "ทริค 3: โค้ดหลัง `execve` คือทางล้มเหลวเสมอ" },
      { p: "ถ้าสำเร็จมันไม่กลับมา — บรรทัดถัดไปจึงเขียนได้แค่ `perror` + `exit(127)`. ไม่ต้องเช็คค่าคืนของมันเลยด้วยซ้ำ" },
      { h: "ทริค 4: ใช้รหัสออกตามมาตรฐาน shell" },
      { p: "`127` = หาคำสั่งไม่เจอ · `126` = เจอแต่รันไม่ได้ · `1` = ทำงานแล้วผิดพลาด. กรรมการเทียบกับ `echo $?` ของ shell จริงได้ทันที" },
      { h: "ทริค 5: rolling pipe สำหรับ bonus" },
      { p: "เก็บ `prev_fd` ไว้แล้วส่งต่อในลูป — คำสั่งที่ i อ่านจาก `prev_fd` และเขียนลงท่อใหม่ จากนั้นปลายอ่านของท่อนั้นกลายเป็น `prev_fd` ของรอบถัดไป. **โค้ดชุดเดียวรองรับ N คำสั่ง**" },
      { h: "ทริค 6: here_doc คือ pipe ที่เราเขียนเอง" },
      { p: "อ่าน stdin จนเจอ LIMITER แล้วเขียนลงปลายเขียนของ pipe — ปลายอ่านกลายเป็น stdin ของคำสั่งแรก. และ outfile ของโหมดนี้ต้องเปิดด้วย `O_APPEND` ไม่ใช่ `O_TRUNC`" },
      { h: "ทริค 7: เขียน `free_tab` ครั้งเดียวแล้วใช้ทุกที่" },
      { p: "`ft_split` คืนอาเรย์ของสตริง — ต้อง `free` ทีละตัวแล้วค่อย `free` อาเรย์. pipex เรียก split ทั้งตอนแยก PATH และตอนแยก argument ของคำสั่ง จึงใช้ซ้ำหลายที่" },
      { h: "ทริค 8: `waitpid` ให้ครบทุกลูก" },
      { p: "รอไม่ครบ = เหลือ zombie และรหัสออกของ process สุดท้ายก็อ่านไม่ได้. เก็บสถานะจาก `waitpid` ของคำสั่งสุดท้ายไปใช้เป็นรหัสออกของ pipex เอง เหมือนที่ shell ทำ" },
    ],
    eval: [
      { qa: [
        ...
        { q: "อธิบายลำดับ open/pipe/fork/dup2/close ของ `< in cmd1 | cmd2 > out`", a: "สร้าง pipe ก่อน → fork ลูกที่ 1: เปิด infile, `dup2` เป็น stdin, `dup2` ปลายเขียนเป็น stdout, ปิด fd ที่เหลือ, exec → fork ลูกที่ 2: `dup2` ปลายอ่านเป็น stdin, เปิด outfile `dup2` เป็น stdout, ปิดที่เหลือ, exec → parent ปิดทั้ง 2 ปลายแล้ว `waitpid` ทั้งคู่" },
        { q: "ทำไม parent ต้องปิดปลายท่อทั้งที่ไม่ได้ใช้เลย?", a: "เพราะ `fork` ก๊อปตาราง fd ไปให้ลูก **และ parent เองก็ยังถืออยู่** — ตราบใดที่ยังมีใครถือปลายเขียน ฝั่งอ่านจะไม่เห็น EOF" },
        { q: "หา path ของคำสั่งยังไง ถ้าไม่มี `PATH` ใน env?", a: "แยก `PATH` ด้วย `:` แล้วต่อกับชื่อคำสั่ง ทดสอบด้วย `access(p, X_OK)` ทีละอัน. ถ้าไม่มี `PATH` เลยหรือหาไม่เจอ → `command not found` แล้ว exit 127 (ยกเว้นชื่อที่มี `/` อยู่แล้วซึ่งใช้ตรง ๆ)" },
        { q: "outfile ควรเปิดด้วย flag อะไร?", a: "`O_WRONLY | O_CREAT | O_TRUNC` โหมดปกติ และ `O_APPEND` แทน `O_TRUNC` เมื่อเป็น here_doc — พร้อม permission `0644`" },
        { q: "รหัสออกของ pipex ควรเป็นของใคร?", a: "ของคำสั่ง**สุดท้าย** เหมือน shell — เก็บสถานะจาก `waitpid` ของลูกตัวสุดท้ายแล้วแปลงด้วย `WEXITSTATUS`" },
        { q: "`access` กับ `X_OK` ต่างจาก `F_OK` ยังไง?", a: "`F_OK` แค่ถามว่ามีไฟล์อยู่ไหม; `X_OK` ถามว่ารันได้ไหม — ไฟล์ที่มีอยู่แต่ไม่มีสิทธิ์ execute ต้องได้รหัสออก 126 ไม่ใช่ 127" },
        { q: "ถ้าคำสั่งใดคำสั่งหนึ่งพัง อีกตัวควรเป็นยังไง?", a: "ยังต้องทำงานต่อ — เหมือน shell. คำสั่งแรกที่ exec ไม่ผ่านจะปิดปลายเขียนตอน exit ทำให้คำสั่งที่สองเห็น EOF แล้วจบตามปกติ" },
      ]},
...
เช็คผล: ยังเหลือ C/E ที่ไม่ใช่ V ไหม? ไม่เหลือ → เดินถึงครบ ✓`, cap: "flood fill = DFS ที่ทิ้งรอย 'V' กันเดินซ้ำ (นี่คือ base case ที่ทำให้ recursion จบ)", lang: "txt" },
      ...
      { qa: [
        { q: "flood fill ทำงานยังไง?", a: "เริ่มจากจุดหนึ่งแล้วแผ่ไปยังช่องข้างเคียงที่เดินได้ ทำเครื่องหมายว่าไปถึงแล้ว และแผ่ต่อจากช่องนั้น — จนไม่มีช่องใหม่ให้ไป. ช่องที่ไม่ถูกทำเครื่องหมายคือช่องที่ไปไม่ถึง" },
        { q: "DFS กับ BFS ต่างกันยังไง ใช้ตัวไหนดี?", a: "DFS ลงลึกก่อน (ใช้ recursion หรือ stack); BFS แผ่เป็นชั้น (ใช้ queue). สำหรับ 'ไปถึงหรือไม่' ทั้งคู่ให้คำตอบเดียวกันและเป็น O(V+E) เท่ากัน — DFS เขียนสั้นกว่าจึงนิยมกว่าในข้อนี้" },
        { q: "ความซับซ้อนของ flood fill บนแผนที่ w×h เท่าไหร่?", a: "O(w·h) — แต่ละช่องถูกเข้าเยี่ยมและทำเครื่องหมายไม่เกิน 1 ครั้ง ส่วนขอบมีจำนวนคงที่ต่อช่อง (4 ทิศ)" },
      ]},
...
→ สำหรับแผนที่ใหญ่มากต้องเปลี่ยนเป็น BFS แบบ queue (วนลูป ไม่ recursion)
  แต่ขนาดแผนที่ในโปรเจกต์เล็กพอ → recursion ปลอดภัยและสั้นกว่า`, cap: "trade-off: recursion สั้น/อ่านง่าย แต่จำกัดด้วยขนาด stack", lang: "txt" },
      ...
      { qa: [
        { q: "recursion ลึกเกินแล้วเกิดอะไร?", a: "stack overflow — แต่ละชั้นกินพื้นที่บน call stack ซึ่งมีขนาดจำกัด (ปกติ 8 MB). โปรแกรมตายด้วย segfault โดยไม่มีคำเตือน" },
        { q: "flood fill บนแผนที่ใหญ่แค่ไหนถึงเสี่ยง?", a: "ความลึกสูงสุดเท่ากับจำนวนช่องที่เดินได้ในเส้นทางเดียว — แผนที่ระดับหลักหมื่นช่องที่เป็นทางเดินยาวต่อกันเริ่มเสี่ยง. แก้ด้วยการเปลี่ยนเป็น BFS ที่ใช้ queue บน heap" },
        { q: "ทำไมทำเครื่องหมายช่องก่อนเรียกซ้ำ ไม่ใช่หลัง?", a: "ถ้าทำหลัง ช่องเดิมจะถูกเข้าซ้ำจากเพื่อนบ้านอีกทางแล้ววนไม่จบทันที — ทำเครื่องหมายก่อนคือสิ่งที่ทำให้ลูปหยุด" },
      ]},
...
แปลงช่อง (x,y) เป็นพิกเซลบนจอ:
  ...
  screen_y = (y − cam_y) × TILE   (y ของจอชี้ลง!)`, cap: "เป็นการ 'เลื่อน frame of reference' — หลักการเดียวกับกล้องในเกมทุกชนิด", lang: "txt" },
      ...
      { qa: [
        { q: "camera คำนวณยังไง?", a: "`cam = ตำแหน่งผู้เล่น − ครึ่งจอ` แล้ว clamp ให้อยู่ในช่วง `0 .. ขนาดแผนที่ − ขนาดจอ` — ทำให้ผู้เล่นอยู่กลางจอยกเว้นตอนใกล้ขอบแผนที่" },
        { q: "ทำไมต้อง clamp?", a: "ไม่งั้นตอนผู้เล่นอยู่มุมแผนที่ กล้องจะเลื่อนออกนอกขอบแล้ววาดพื้นที่ที่ไม่มีข้อมูล — ได้ขอบดำหรืออ่านนอกอาเรย์" },
        { q: "แปลงพิกัดแผนที่เป็นพิกัดจอยังไง?", a: "`screen_x = (map_x − cam_x) × TILE` — ลบตำแหน่งกล้องออกก่อนแล้วคูณขนาดกระเบื้อง" },
      ]},
...
δ  (transition):
   ...
   กำลังเล่น --กดทิศ (ไม่ชน)---> กำลังเล่น (อัปเดตตำแหน่ง)`, cap: "การคิดเป็น FSM ทำให้ logic ชัด ครบทุกกรณี — แนวคิดเดียวกับ state machine ในระบบใหญ่ (เช่น LangGraph)", lang: "txt" },
      { qa: [
        { q: "เกมนี้มีสถานะอะไรบ้าง?", a: "กำลังเล่น (ยังเก็บไม่ครบ) → เก็บครบแล้ว (ทางออกเปิด) → ชนะ. การเปลี่ยนสถานะเกิดจากเหตุการณ์เดียวคือการเดินของผู้เล่น" },
        { q: "เข้า exit ก่อนเก็บครบต้องเป็นยังไง?", a: "ไม่เกิดอะไรเลย — ยังอยู่สถานะเดิม. ทางออกทำตัวเหมือนกำแพงจนกว่า `collected == total`" },
        { q: "ทำไมเขียนเป็น FSM ถึงดีกว่าใส่ `if` กระจาย?", a: "เพราะเงื่อนไขการชนะอยู่ที่เดียว — เพิ่มสถานะใหม่ (เช่นศัตรูที่ทำให้แพ้) ก็แค่เพิ่มการเปลี่ยนสถานะ ไม่ต้องไล่แก้ `if` ทุกจุดที่ตรวจการเดิน" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
mlx_loop(game.mlx);`, cap: "main.c: ผูก event แล้วเข้า loop", lang: "c" },
    ...
    tricks: [
      { h: "ทริค 1: flood fill บนสำเนาเสมอ" },
      { p: "ตรวจว่าเดินถึงทุกอย่างไหมโดยไม่ทำลายแผนที่จริง — ก๊อป grid ก่อนแล้วเติมบนสำเนา. ถ้าทำบนตัวจริงต้องมาคืนค่าเดิมทีหลังซึ่งพลาดง่ายกว่ามาก" },
      { h: "ทริค 2: แทน `P` ด้วย `0` ทันทีหลังอ่านแผนที่" },
      { p: "เก็บพิกัดผู้เล่นไว้ใน `px`/`py` แล้วให้ช่องนั้นเป็นพื้นธรรมดา — ตอนวาดและตอนเดินไม่ต้องมีเคสพิเศษ 'ช่องนี้มีผู้เล่นอยู่' อีกเลย" },
      { h: "ทริค 3: วาดพื้นก่อนเสมอ แล้วค่อยวาดของทับ" },
      { p: "`.xpm` ที่มีพื้นโปร่งใสจะเห็นสิ่งที่อยู่ข้างหลัง — ถ้าไม่วาดพื้นรองไว้ก่อนจะเห็นเศษของเฟรมก่อนหน้าค้างอยู่" },
      { h: "ทริค 4: camera = ผู้เล่น − ครึ่งจอ แล้ว clamp" },
      { p: "สูตรสองส่วนนี้รองรับแผนที่ที่ใหญ่กว่าหน้าต่างได้ทันที และการ clamp คือสิ่งที่กันไม่ให้กล้องเลื่อนออกนอกขอบแผนที่" },
      { h: "ทริค 5: event 17 คือปุ่ม X ของหน้าต่าง" },
      { p: "`DestroyNotify` ต้อง hook แยกจากปุ่ม ESC — ไม่ hook แล้วกดกากบาทจะเหลือ process ค้างหรือ crash ซึ่งกรรมการลองแน่นอน" },
      { h: "ทริค 6: ตรวจแผนที่ให้ครบก่อนเปิดหน้าต่าง" },
      { p: "นามสกุล `.ber` · เป็นสี่เหลี่ยมผืนผ้า · กำแพงล้อมรอบ · มี `P` 1 ตัว `E` อย่างน้อย 1 `C` อย่างน้อย 1 · ไม่มีอักขระแปลก · เดินถึงทุก `C` และ `E`. เปิดหน้าต่างก่อนแล้วค่อยพบว่าแผนที่พังคือ error ที่เก็บกวาดยากกว่า" },
      { h: "ทริค 7: ปล่อยทรัพยากร MiniLibX ให้ครบตามลำดับ" },
      { p: "`mlx_destroy_image` ทุกภาพ → `mlx_destroy_window` → `mlx_destroy_display` → `free(mlx)` แล้วค่อย `free` แผนที่. สลับลำดับแล้ว valgrind จะฟ้อง invalid read" },
      { h: "ทริค 8: นับก้าวแล้วพิมพ์ทุกครั้งที่เดินสำเร็จ" },
      { p: "นับเฉพาะการเดินที่เกิดขึ้นจริง — ชนกำแพงไม่นับ. subject บอกให้แสดงในเทอร์มินัล ส่วนการวาดลงหน้าต่างเป็น bonus" },
    ],
    eval: [
      { qa: [
        ...
        { q: "แผนที่ต้องผ่านการตรวจกี่ข้อ?", a: "นามสกุล `.ber`, เป็นสี่เหลี่ยมผืนผ้า (ทุกแถวยาวเท่ากัน), กำแพง `1` ล้อมรอบทั้ง 4 ด้าน, มี `P` เพียง 1 ตัว, มี `E` และ `C` อย่างน้อยอย่างละ 1, ไม่มีอักขระนอกเหนือ `01PEC`, และเดินถึง `C` ทุกตัวกับ `E` ได้" },
        { q: "แผนที่ที่มี `P` 2 ตัวควรเป็นยังไง?", a: "Error — subject กำหนดให้มีผู้เล่นตัวเดียวเท่านั้น" },
        { q: "ตรวจว่าเป็นสี่เหลี่ยมผืนผ้ายังไง?", a: "จำความยาวของแถวแรกไว้แล้วเทียบกับทุกแถว — ระวังตัด `\n` ท้ายบรรทัดออกก่อนวัด ไม่งั้นแถวสุดท้ายที่ไม่มี newline จะสั้นกว่าเสมอ" },
        { q: "อ่านแผนที่ด้วยอะไร?", a: "`get_next_line` ทีละบรรทัดแล้วต่อกัน — อ่านทั้งไฟล์ทีเดียวไม่ได้เพราะไม่รู้ขนาดล่วงหน้า และ `get_next_line` เป็นสิ่งที่เขียนไว้แล้วในโปรเจกต์ก่อนหน้า" },
        { q: "ทำไมต้องล้อมด้วยกำแพง?", a: "เพื่อให้การตรวจการเดินไม่ต้องเช็คขอบอาเรย์ทุกครั้ง — ทุกช่องที่เดินได้มีเพื่อนบ้านครบ 4 ทิศเสมอ" },
        { q: "ภาพ `.xpm` โหลดยังไง และต้องระวังอะไร?", a: "`mlx_xpm_file_to_image` คืน pointer พร้อมความกว้าง/สูง — ต้องเช็ค NULL ทุกภาพ (ไฟล์หายหรือรูปแบบผิด) และต้อง `mlx_destroy_image` ทุกตัวตอนจบ" },
        { q: "ทำไมไม่วาดใหม่ทั้งจอทุกเฟรม?", a: "so_long ไม่มีลูปเรนเดอร์ตลอดเวลา — วาดใหม่เฉพาะตอนมีการเดิน. วาดทั้งจอตอนนั้นก็ยังเร็วพอเพราะแผนที่เล็ก" },
        { q: "จัดการ leak ยังไง?", a: "ทุกทางออก (แผนที่ผิด, โหลดภาพไม่ได้, กด ESC, กดกากบาท) ต้องผ่านฟังก์ชันเก็บกวาดตัวเดียวกัน — ทำ `cleanup(game)` ตัวเดียวแล้วเรียกจากทุกที่" },
      ]},
      { h: "ทดสอบ" },
...
z² + c = (a² − b² + e) + (2ab + f)i
  ...
  |z₁|² = 0.86² + 0.57² = 0.74 + 0.32 = 1.06  (< 4 → ยังไม่หนี วนต่อ)`, cap: "วน z₁→z₂→... จนกว่า |z|²>4 หรือครบ max_iter", lang: "txt" },
      ...
      { qa: [
        { q: "`z²` ของจำนวนเชิงซ้อนคำนวณยังไง?", a: "`(a + bi)² = a² − b² + 2abi` — ส่วนจริงคือ `a² − b²` ส่วนจินตภาพคือ `2ab`. เครื่องหมายลบมาจาก `i² = −1`" },
        { q: "ทำไมต้องเก็บค่าเก่าไว้ก่อนอัปเดต?", a: "เพราะส่วนจินตภาพใหม่ใช้ส่วนจริง**เก่า** — เขียนทับ `re` ก่อนแล้วคำนวณ `im` จะได้ค่าผิดทันที. เก็บ `tmp = re` ไว้ก่อนเสมอ" },
        { q: "ทำไมไม่ใช้ `complex.h` ของ C99?", a: "ใช้ได้ในทางเทคนิค แต่การเขียนเองทำให้เห็นว่าแต่ละรอบทำอะไรและควบคุมประสิทธิภาพได้ — ลูปนี้รันหลายล้านครั้งต่อเฟรม" },
      ]},
...
→ ตัด sqrt ทิ้ง = เร็วขึ้นมาก (รันหลายล้านครั้ง/เฟรม)`, lang: "txt" },
      ...
      { qa: [
        { q: "ทำไมใช้รัศมี 2 เป็นเกณฑ์หนี?", a: "พิสูจน์ได้ว่าถ้า `|z| > 2` แล้ว `|z|` จะโตขึ้นเรื่อย ๆ ไม่มีทางกลับเข้ามา — จึงสรุปได้ทันทีว่าจุดนั้นไม่อยู่ในเซต โดยไม่ต้องวนต่อ" },
        { q: "ใช้ค่ามากกว่า 2 ได้ไหม?", a: "ได้และผลยังถูก แต่ต้องวนมากขึ้นโดยไม่ได้อะไรเพิ่ม — 2 คือค่าที่เล็กที่สุดที่ยังรับประกันความถูกต้อง" },
        { q: "ใช้ค่าน้อยกว่า 2 ได้ไหม?", a: "ไม่ได้ — จุดที่ `|z|` แตะ 1.9 แล้ววกกลับเข้ามาอยู่ในเซตมีจริง การตัดที่ค่านั้นจะระบายสีผิด" },
      ]},
...
→ เหตุผลที่ใช้ double ไม่ใช่ float (float แตกเร็วกว่ามาก ~2⁻²³)
  และเหตุผลที่การซูมมี "เพดาน" ตามธรรมชาติ`, cap: "ข้อจำกัดเชิงตัวเลขนี้เป็นของจริงในทุก fractal renderer", lang: "txt" },
      ...
      { qa: [
        { q: "ทำไมซูมลึกมาก ๆ แล้วภาพแตกเป็นบล็อก?", a: "`double` มีความละเอียดจำกัด (~15-16 หลักทศนิยม) — เมื่อความกว้างของกรอบเล็กกว่าระยะห่างระหว่าง 2 ค่าที่แทนได้ พิกเซลข้างเคียงจะได้ค่าเดียวกันเป๊ะ จึงกลายเป็นบล็อกสีเดียว" },
        { q: "แก้ได้ไหม?", a: "ต้องเปลี่ยนไปใช้เลขความละเอียดสูง (`long double` ช่วยได้อีกนิด, arbitrary precision ช่วยได้จริงแต่ช้ามาก) — นอกขอบเขตของโปรเจกต์ แต่ควรอธิบายสาเหตุได้" },
        { q: "`float` ใช้แทน `double` ได้ไหม?", a: "ได้แต่ซูมได้ตื้นกว่ามาก (~7 หลัก) — เห็นภาพแตกเร็วกว่าหลายเท่า" },
      ]},
...
แกน im กลับด้าน:  im = max_im − (y/HEIGHT)(max_im − min_im)
   เพราะ y ของจอเพิ่มลงล่าง แต่แกนจินตภาพเพิ่มขึ้นบน`, cap: "interpolation เชิงเส้น = แมพค่าจากช่วงหนึ่งไปอีกช่วงแบบสัดส่วน ใช้ทั้ง render และ zoom", lang: "txt" },
      ...
      { qa: [
        { q: "แปลงพิกเซลเป็นจุดบนระนาบเชิงซ้อนยังไง?", a: "สเกลเชิงเส้น: `re = min_re + x × (max_re − min_re) / width` และ `im = max_im − y × (max_im − min_im) / height`" },
        { q: "ทำไมสูตรแกนจินตภาพถึงเป็นการลบ?", a: "เพราะแกน y ของหน้าจอเพิ่มลงล่าง แต่แกนจินตภาพเพิ่มขึ้นบน — ต้องกลับด้าน ไม่งั้นภาพกลับหัว" },
        { q: "ทำไมต้องรักษาอัตราส่วนภาพ?", a: "ถ้าช่วงของแกนจริงกับแกนจินตภาพไม่ได้สัดส่วนกับความกว้าง/สูงของหน้าต่าง ภาพจะถูกยืดหรือบีบ — เซต Mandelbrot จะดูผิดรูป" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
if (iter >= f->max_iter)
    return (0x000000);                          // อยู่ในเซต = ดำ
t = (double)((iter + f->shift) % f->max_iter) / f->max_iter;
r = (int)(9.0  * (1-t) * t*t*t       * 255);    // ต้อง cast เป็น int
g = (int)(15.0 * (1-t)*(1-t) * t*t   * 255);    // ก่อนเอาไป << เสมอ
b = (int)(8.5  * (1-t)*(1-t)*(1-t)*t * 255);
return ((r << 16) | (g << 8) | b);`, cap: "polynomial palette ให้ไล่สีนุ่ม; ต้อง cast เป็น int ก่อน bit shift — shift บน double คอมไพล์ไม่ผ่าน", lang: "c" },
    ],
    tricks: [
      { h: "ทริค 1: เทียบ `|z|² > 4` แทนการเรียก `sqrt`" },
      { p: "ยกกำลังสองทั้ง 2 ข้างของ `|z| > 2` แล้วได้เงื่อนไขเดียวกันโดยไม่ต้องถอดรากเลย — `sqrt` ถูกเรียกหลายล้านครั้งต่อเฟรม การตัดออกเห็นผลชัด" },
      { h: "ทริค 2: เขียนลงบัฟเฟอร์ของภาพตรง ๆ" },
      { p: "คำนวณ offset เอง (`addr + y × line_len + x × (bpp / 8)`) แล้ว `mlx_put_image_to_window` ทีเดียวตอนจบ — เร็วกว่า `mlx_pixel_put` ทีละจุดหลักร้อยเท่าเพราะไม่ต้องคุยกับ X server ทุกพิกเซล" },
      { h: "ทริค 3: Mandelbrot กับ Julia ใช้ลูปเดียวกัน" },
      { p: "ต่างกันแค่ค่าเริ่มต้น — Mandelbrot: `z = 0`, `c = จุดของพิกเซล`; Julia: `z = จุดของพิกเซล`, `c = ค่าคงที่`. เขียนฟังก์ชันเดียวแล้วส่ง 2 ค่านี้เข้าไป" },
      { h: "ทริค 4: ซูมรอบตำแหน่งเมาส์ ไม่ใช่รอบกลางจอ" },
      { p: "แปลงพิกัดเมาส์เป็นจุดเชิงซ้อนก่อน แล้วสเกลกรอบโดยตรึงจุดนั้นไว้กับที่ — ทำให้ซูมแล้วสิ่งที่ชี้อยู่ไม่เลื่อนหนี ซึ่งเป็นสิ่งที่กรรมการลองทันที" },
      { h: "ทริค 5: กลับด้านแกนจินตภาพ" },
      { p: "`im = max_im − y × step` ไม่ใช่ `min_im + ...` — แกน y ของหน้าจอเพิ่มลงล่างแต่แกนจินตภาพเพิ่มขึ้นบน. ลืมแล้วภาพกลับหัวโดยที่รูปทรงยังดูถูก จับได้ยาก" },
      { h: "ทริค 6: จำกัดช่วงของ `max_iter`" },
      { p: "ต่ำเกินไปภาพหยาบ สูงเกินไปเฟรมค้าง — clamp ไว้ราว 20 ถึง 1000 แล้วให้ผู้ใช้ปรับด้วย `+`/`-`. เพิ่ม iteration ตอนซูมลึกคือสิ่งที่ทำให้รายละเอียดยังโผล่มา" },
      { h: "ทริค 7: ปล่อยทรัพยากร MiniLibX ให้ครบตอนปิด" },
      { p: "`mlx_destroy_image` → `mlx_destroy_window` → `mlx_destroy_display` → `free(mlx)`. ต้องทำทั้งตอนกด ESC และตอนกดกากบาท (event 17) — ทางออก 2 ทางต้องเรียกฟังก์ชันเก็บกวาดตัวเดียวกัน" },
      { h: "ทริค 8: ไล่สีจากจำนวนรอบ ไม่ใช่จาก 'อยู่/ไม่อยู่'" },
      { p: "จุดที่หนีเร็วกับหนีช้าให้สีต่างกัน — นี่คือสิ่งที่ทำให้เห็นลวดลายรอบขอบเซต. ระบายแค่ 2 สีจะได้ภาพทึบที่ไม่มีรายละเอียดอะไรเลย" },
    ],
    eval: [
      { qa: [
        ...
        { q: "เซต Mandelbrot นิยามว่าอะไร?", a: "เซตของ `c` ที่ทำให้ลำดับ `z → z² + c` โดยเริ่มจาก `z = 0` **ไม่หนีไปอนันต์** — จุดในเซตระบายสีหนึ่ง จุดนอกเซตระบายตามจำนวนรอบที่ใช้ก่อนหนี" },
        { q: "Julia set ต่างจาก Mandelbrot ตรงไหน?", a: "สลับบทบาทกัน — Julia ตรึง `c` ไว้เป็นค่าคงที่แล้วให้ `z` เริ่มจากจุดของพิกเซล. `c` แต่ละค่าให้ Julia set คนละรูป ซึ่งเป็นที่มาของโหมดที่ `c` เปลี่ยนตามเมาส์" },
        { q: "ต้องทำแฟรกทัลกี่แบบ?", a: "อย่างน้อย 2 (Mandelbrot และ Julia) และส่วนบังคับให้เพิ่มอีกอย่างน้อย 1 แบบ เช่น Burning Ship ซึ่งต่างจาก Mandelbrot แค่ใส่ค่าสัมบูรณ์ให้ทั้ง 2 ส่วนก่อนยกกำลังสอง" },
        { q: "โปรแกรมรับ argument ยังไง และผิดแล้วทำอะไร?", a: "`./fractol mandelbrot` หรือ `./fractol julia <re> <im>` — argument ไม่ถูกต้องต้องพิมพ์วิธีใช้แล้วออกอย่างเรียบร้อย ไม่ใช่ crash" },
        { q: "ทำไมภาพยังเรนเดอร์ทันทั้งที่วนทุกพิกเซล?", a: "เขียนลงบัฟเฟอร์ในหน่วยความจำแล้วส่งทั้งภาพครั้งเดียว บวกกับการเลี่ยง `sqrt` และการหยุดลูปทันทีที่หนี — จุดส่วนใหญ่หนีภายในไม่กี่รอบ มีแค่จุดในเซตเท่านั้นที่วนครบ `max_iter`" },
        { q: "ปุ่มและเมาส์ทำอะไรบ้าง?", a: "ล้อเมาส์ซูมเข้า/ออกรอบตำแหน่งเคอร์เซอร์ · ลูกศรเลื่อนภาพ · `+`/`-` ปรับจำนวน iteration · ESC และปุ่มกากบาทปิดโปรแกรมพร้อมคืนทรัพยากร" },
        { q: "ทำไมต้อง hook ทั้ง ESC และ event 17?", a: "เป็นทางออกคนละทาง — ปิดด้วยกากบาทโดยไม่ hook `DestroyNotify` จะได้ process ค้างหรือ crash ซึ่งกรรมการลองแน่นอน" },
        { q: "ถ้าซูมลึกจนภาพแตกจะอธิบายกรรมการยังไง?", a: "เป็นขีดจำกัดของ `double` ตามมาตรฐาน IEEE 754 — ที่ความละเอียดประมาณ 10⁻¹⁵ พิกเซลข้างเคียงเริ่มได้ค่าเดียวกันเป๊ะ. รู้สาเหตุและอธิบายได้สำคัญกว่าแก้ได้" },
      ]},
      { h: "ทดสอบ" },
...
วิธีแก้ใน bonus:      รอ ack ทุก bit (lock-step) — แก้ที่ราก`, cap: "นี่คือเหตุผลที่ต้องมี usleep (mandatory) หรือ ack handshake (bonus) — ไม่ใช่ของแถม แต่จำเป็น", lang: "txt" },
      ...
      { qa: [
        { q: "ทำไมสัญญาณถึงหล่นหาย?", a: "สัญญาณมาตรฐานไม่เข้าคิว — ระบบเก็บได้แค่ 'ค้างอยู่หรือไม่' เป็นบิตเดียวต่อชนิด. ส่ง SIGUSR1 ซ้ำก่อนตัวแรกถูกจัดการ ตัวที่สองหายไปเลย" },
        { q: "แก้ยังไง?", a: "ต้องมีการตอบรับ — ผู้รับส่งสัญญาณกลับไปบอกว่า 'รับบิตนี้แล้ว' และผู้ส่งรอสัญญาณนั้นก่อนส่งบิตถัดไป. การหน่วงเวลาแบบเดา ๆ ไม่ใช่คำตอบเพราะเครื่องเร็วช้าไม่เท่ากัน" },
        { q: "`sigaction` ดีกว่า `signal` ยังไง?", a: "พฤติกรรมนิยามชัดเจนและพกพาได้ (`signal` ต่างกันไปตามระบบ), ตั้งหน้ากากบล็อกสัญญาณระหว่างตัวจัดการทำงานได้, และรับข้อมูลเพิ่มอย่าง PID ผู้ส่งได้ผ่าน `SA_SIGINFO`" },
      ]},
...
ลองกับ bit = 1:
  (0100 0001 >> 1) & 1 = 0010 0000 & 1 = 0 → SIGUSR1 ✓`, cap: ">>(เลื่อนขวา) ย้ายบิตเป้ามาตำแหน่ง 0, &1 mask เอาเฉพาะตัวนั้น — ได้ 0 หรือ 1 พอดี", lang: "txt" },
      { code: String.raw`ฝั่ง server — ประกอบ 'A' กลับด้วย c = c << 1 | bit:
(client ส่ง MSB→LSB ของ 0100 0001 = U1 U2 U1 U1 U1 U1 U1 U2)
...
ผล = 0100 0001 = 65 = 'A' ✓ — ครบ 1 byte`, cap: "เลื่อนซ้าย = เปิดที่ว่างขวาสุด, |1 เติมบิตใหม่ลงไป — MSB-first ทำให้ลำดับถูกต้องโดยอัตโนมัติ", lang: "txt" },
      { note: "ทำไมต้อง MSB-first: ถ้าส่ง LSB-first server จะต้อง 'รู้' ว่าตอนนี้อยู่ bit ที่เท่าไรแล้วจึงจะรู้ว่าจะ `|` ค่าที่ตำแหน่งไหน — โค้ดซับซ้อนขึ้น. MSB-first + `<<1` ทำให้ 'ไม่ต้องนับตำแหน่ง' เลย — bit ไปนั่งถูกเอง" },

      { qa: [
        { q: "ดึงบิตที่ i ของไบต์ออกมายังไง?", a: "`(c >> i) & 1` — เลื่อนบิตที่ต้องการมาอยู่ขวาสุดแล้วปิดบิตอื่นทั้งหมดด้วย 1" },
        { q: "ประกอบบิตกลับเป็นไบต์ยังไง?", a: "`byte = (byte << 1) | bit` ทำ 8 ครั้ง — เลื่อนของเดิมไปทางซ้ายแล้วเติมบิตใหม่ที่ตำแหน่งขวาสุด" },
        { q: "ส่งจากบิตสูงไปต่ำหรือต่ำไปสูงดีกว่า?", a: "อย่างไหนก็ได้ ขอให้ทั้งสองฝั่งตกลงตรงกัน — ผิดฝั่งเดียวได้ตัวอักษรที่บิตกลับด้าน เช่น 'A' (65) กลายเป็น 130" },
      ]},
...
static void handle_signal(int sig)
{
    ...
    if (bits == 8)
    {
      ...
      { note: "ใน minitalk เราโชคดีที่ ft_printf สร้างบน write() — ถ้าโปรเจกต์อื่นต้อง print ใน handler ให้ตรวจสอบเสมอว่าฟังก์ชัน print นั้นสร้างบน write() จริง ๆ ไม่ใช่ libc printf" },

      { qa: [
        { q: "async-signal-safe แปลว่าอะไร?", a: "ฟังก์ชันที่เรียกจากในตัวจัดการสัญญาณได้อย่างปลอดภัย — เพราะสัญญาณแทรกกลางการทำงานปกติได้ทุกจังหวะ รวมทั้งตอนที่ `malloc` กำลังแก้โครงสร้างภายในอยู่" },
        { q: "`printf` ในตัวจัดการสัญญาณอันตรายยังไง?", a: "มันใช้บัฟเฟอร์ภายในและอาจ `malloc` — ถ้าสัญญาณแทรกตอนโปรแกรมหลักอยู่กลาง `printf` พอดี โครงสร้างภายในจะพังหรือ deadlock" },
        { q: "แล้วใช้อะไรแทน?", a: "`write` ซึ่งอยู่ในรายการที่ปลอดภัย — หรือเก็บสถานะไว้ในตัวแปร `volatile sig_atomic_t` แล้วให้ลูปหลักเป็นคนพิมพ์" },
      ]},
...
server รับ 4 bytes → write → terminal แสดง '🎉' ✓`, lang: "txt" },
      { note: "ทำไมพิมพ์ทีละ byte แล้วยังเห็นภาษาไทย/emoji ครบ: `ft_printf(\"%c\", c)` เขียน byte ดิบออก stdout ทีละตัวผ่าน `write` ไม่ได้ตีความว่าเป็นตัวอักษรอะไร. terminal ฝั่งรับเป็นคนสะสม byte เอง พอครบลำดับ UTF-8 ของตัวนั้นก็เรนเดอร์ออกมา — server จึงไม่ต้องรู้จัก UTF-8 เลย" },

      { qa: [
        { q: "ทำไมส่งทีละไบต์แล้วภาษาไทยยังแสดงถูก?", a: "UTF-8 เก็บอักขระหนึ่งตัวเป็นหลายไบต์เรียงกัน — ถ้าส่งครบทุกไบต์ตามลำดับ เทอร์มินัลจะประกอบกลับเองตอนแสดงผล. เราไม่ต้องรู้เลยว่าตัวไหนกินกี่ไบต์" },
        { q: "อักขระไทยหนึ่งตัวใช้กี่ไบต์?", a: "3 ไบต์ใน UTF-8 (อังกฤษ 1 ไบต์, อีโมจิส่วนใหญ่ 4 ไบต์) — จำนวนบิตที่ต้องส่งจึงเป็น 3 เท่าของจำนวนอักขระ" },
        { q: "ต้องส่งไบต์ปิดท้ายด้วยไหม?", a: "ต้อง — ส่ง `\0` เป็นสัญญาณจบข้อความ ไม่งั้นผู้รับไม่รู้ว่าควรพิมพ์ตอนไหนและข้อความถัดไปจะต่อกัน" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
วนแต่ละก้าว:
  ...
  จนถึงปลายทาง`, cap: "error บอกว่าตอนนี้เส้นจริง 'เอียง' ไปทางไหน → เลือกขยับแกนที่ทำให้ใกล้เส้นจริงสุด", lang: "txt" },
      ...
      { p: "isometric คือการวาดแกน 3 แกนของโลกให้เอียง 30° เท่า ๆ กันบนจอ. แทนที่จะท่องสูตร `(x−y)cos30` มาดูว่ามัน 'โผล่ออกมา' จากการวางทิศของแต่ละแกนยังไง" },
...
จุดสูง (x=0, y=0, z=3):
  ...
  screen_y = 0 − 3·scale              → ขยับ "ขึ้น" (ค่า y ติดลบ = สูงขึ้นบนจอ) ✓`, cap: "z ลบออกจาก screen_y เพราะจอ y ชี้ลง — จุดยิ่งสูงยิ่งต้องไปทางบน (y น้อย)", lang: "txt" },
      ...
      { p: "เส้นตรงคณิตศาสตร์เป็นทศนิยม (y = mx + c) แต่จอมี pixel เป็นช่องจำนวนเต็ม. Bresenham เลือก pixel ที่ 'ใกล้เส้นจริงสุด' ทีละก้าวโดยใช้แค่บวก/ลบจำนวนเต็ม — เร็วและไม่มีรู" },
      ...
      { code: String.raw`dx =  abs(x1-x0)      sx = (x0 < x1) - (x0 > x1)   // +1 / -1 แบบไม่ใช้ ?:
...
loop:
  ...
  → pixels: (0,0)(1,0)(2,1)(3,1)(4,2)(5,2) = บันไดเอียงเนียนพอดี`, cap: "เส้น slope 2/5: ขยับ x ทุกก้าว, ขยับ y เฉพาะก้าวที่ error สะสมพอ → ได้ขั้นบันไดที่ใกล้เส้นจริงสุด", lang: "txt" },
      ...
      { p: "ปลายเส้นข้างหนึ่งสีแดง อีกข้างสีน้ำเงิน อยากให้ค่อย ๆ ไล่เฉดกลางเส้น. ทำด้วย **linear interpolation**: ที่สัดส่วน t (0→1) ตามแนวเส้น สี = ผสมแดงกับน้ำเงินตาม t" },
...
→ (127<<16)|(0<<8)|127 = 0x7F007F  (ม่วงกลางทาง ✓)`, cap: "ถ้า lerp ทั้งก้อน 0xFF0000↔0x0000FF จะได้ค่าผิดเพราะ carry ข้ามช่อง — ต้องแยกช่องเท่านั้น", lang: "txt" },
      ...
      { p: "แผนที่ 3×3 กับ 100×100 ต้องเต็มจอพอ ๆ กัน. เคล็ดคือ 3 ขั้น: **(1) เลื่อนจุดกึ่งกลางไป origin** (centering) **(2) เลือก zoom ที่พอดี** **(3) บวก offset ไปกลางจอ**" },
...
ขั้น 3 offset — ดันภาพไปกลางจอ:
  off_x = WIN_W/2 ;  off_y = WIN_H/2`, cap: "centering ก่อน ทำให้ zoom/rotate หมุนรอบจุดกึ่งกลาง ไม่เลื่อนหนีมุมจอ", lang: "txt" },
      ...
      { note: "ลองเอง: คอมเมนต์บรรทัด `− width/2` ออก แล้วรัน — ภาพจะกองมุมซ้ายบนและพอ zoom จะหลุดจอทันที. นี่คือ 'หลักฐานเชิงประจักษ์' ว่าทำไม centering จำเป็น" },
      { qa: [
      ...
      { code: String.raw`void draw_line(t_fdf *f, t_pt a, t_pt b)
{
    ...
    while (1)
    {
        t = 0.0;                            // (concept) สัดส่วนตามแนวเส้น
        if (steps != 0)
            t = (double)i / steps;
...
./fdf maps/mars.fdf`, lang: "bash" },
    ...
    eval: [
      { qa: [
        ...
        { q: "ทำไม map ใช้ `int **` ไม่ใช่อาเรย์คงที่?", a: "ขนาดแผนที่ไม่รู้ล่วงหน้า ต้องจองแบบ dynamic ตามไฟล์ — `int **` คืออาเรย์ของ pointer แต่ละตัวชี้ 1 แถว เข้าถึงด้วย `z[y][x]`" },
...
ถ้าทำซ้อนกัน: ควรได้ 7 แต่ได้ 6 → ข้อมูลหาย!`, cap: "การ ++ ไม่ใช่ atomic — มันคือ อ่าน-บวก-เขียน 3 ขั้น ที่ถูกแทรกได้", lang: "txt" },
    ...
    return (tv.tv_sec * 1000L + tv.tv_usec / 1000);
      ...
      { p: "5 คนนั่งโต๊ะกลม มีส้อมคั่นระหว่างทุกคน (5 อัน). ถ้า **ทุกคนหยิบส้อมซ้ายพร้อมกัน** → ทุกคนถือส้อมซ้ายไว้ แล้วเอื้อมขวาไปหยิบส้อมที่ 'คนข้าง ๆ ถืออยู่' → ทุกคนค้างรอกันเป็นวงกลม ไม่มีใครยอมวาง = ค้างทั้งโต๊ะตลอดกาล. นั่นคือ deadlock" },
...
วงจรรอ (เมื่อทุกคนหยิบซ้ายก่อนพร้อมกัน):
  ...
  → ทุกคนได้กิน ไม่มีจังหวะไหนที่ RAG ปิดเป็นวงเลย ✓`, cap: "ทุก tick มี 'ผู้ชนะ' ที่ได้ส้อมครบเสมอ → ไม่มีสภาพ 'ทุกคนถือ 1 อันแล้วค้าง' → cycle ไม่เคยปิด", lang: "txt" },
      ...
      { p: "2 thread แตะตัวแปรเดียวกัน 'พร้อมกัน' โดยไม่มีใครกั้น = เหมือน 2 คนเขียนกระดานแผ่นเดียวทับกัน — ผลที่ออกมาขึ้นกับจังหวะ CPU ที่คาดเดาไม่ได้" },
...
ถ้า monitor แทรกอ่าน last_meal กลางคัน:
  ...
  → กิน 2 มื้อ แต่ meals_eaten = 5 (ควรเป็น 6) → must_eat เพี้ยน`, cap: "load/compute/store ของ 2 thread 'สอดไส้' กัน — mutex บังคับให้ 3 ขั้นนี้ทำจบก่อนอีก thread จะเริ่ม", lang: "txt" },
      { p: "**ทำไม monitor ตัดสินตายผิดได้ แม้ `last_meal` จะเขียนทีเดียวจบ:** บน x86-64 การอ่าน/เขียน `long` 64-bit ที่ align แล้วเป็น atomic ในระดับ CPU จริง — เราจะไม่ได้ค่า 'ครึ่งเก่าครึ่งใหม่'. แต่ atomic ระดับ CPU ยังไม่พอ เพราะปัญหาอยู่ที่ **compiler** และ **ลำดับของการอ่านหลายค่า**:" },
      { code: String.raw`ปัญหาที่ 1 — compiler เก็บค่าไว้ใน register
  monitor เขียนว่า:  while (get_time() - philo->last_meal < t_die) ...
  compiler เห็นว่าไม่มีใครในฟังก์ชันนี้แก้ last_meal
  → โหลดครั้งเดียวแล้วใช้ค่าเดิมวนทุกรอบ
  → เจ้าตัวกินไปแล้ว 10 มื้อ monitor ยังเห็นค่าเก่า → ประกาศตายผิด

ปัญหาที่ 2 — อ่านหลายค่าโดยไม่ atomic ร่วมกัน
  monitor อ่าน last_meal (ค่า A) แล้วอ่าน meals_eaten (ค่า B)
  ระหว่างสองบรรทัดนั้น เจ้าตัวกินเสร็จพอดี
  → ได้ A เก่า คู่กับ B ใหม่ = ภาพที่ไม่เคยมีอยู่จริง

ปัญหาที่ 3 — ตามมาตรฐาน C มันคือ data race = undefined behavior
  แปลว่า compiler มีสิทธิ์ทำอะไรก็ได้ ไม่ใช่แค่ 'ได้ค่าเก่า'`, cap: "การล็อกไม่ได้มีไว้กัน 'ค่าครึ่ง ๆ' อย่างเดียว — มันบังคับให้ compiler อ่านจาก memory จริง และทำให้การอ่านหลายค่าเป็นภาพเดียวที่สอดคล้องกัน", lang: "txt" },
...
// แล้วค่อยตัดสินใจ 'นอก' ล็อก จากค่าที่ copy ออกมาแล้ว`, cap: "กฎเหล็ก: อ่านและเขียน shared ตัวเดียวกัน ต้องใช้ mutex ดอกเดียวกันเสมอ ไม่งั้นยังชนได้", lang: "c" },
      ...
      { qa: [
        { q: "ทำไมแค่ 'อ่าน' อย่างเดียว (monitor ไม่ได้เขียน) ก็ยังต้องล็อก?", a: "เพราะ 'อ่านขณะอีก thread กำลังเขียน' นับเป็น data race ตามมาตรฐาน C = undefined behavior. ผลที่เจอบ่อยสุดคือ compiler เก็บค่าไว้ใน register แล้วใช้ค่าเก่าวนทั้งลูป ทำให้ monitor ไม่เห็นว่าเจ้าตัวเพิ่งกิน — ไม่ใช่เรื่อง 'ค่าครึ่ง ๆ'" },
      ...
      { p: "mutex = กุญแจห้องน้ำ 1 ดอก (เข้าได้ทีละคน คนที่ล็อกต้องเป็นคนปลดเอง). semaphore = ตัวนับที่นั่งในร้าน (มี k ที่นั่ง ใครเข้าก็ลดเคาน์เตอร์ 1 เต็มแล้วรอ — ใครปล่อยก็ได้)" },
...
กฎคร่าว ๆ: slice ควร << เวลาที่สั้นที่สุดในโจทย์ (มัก = time_to_die)`, cap: "slice size = trade-off ระหว่างความแม่นของเวลา (ยิ่งเล็กยิ่งแม่น) กับการใช้ CPU (ยิ่งเล็กยิ่งเปลือง)", lang: "txt" },
      ...
      { qa: [
        { q: "`usleep` ไม่แม่นยังไง?", a: "มันรับประกันแค่ว่า 'อย่างน้อย' ตามเวลาที่ขอ — ตัวจัดคิวของระบบอาจปลุกช้ากว่านั้นหลายมิลลิวินาที ซึ่งเกินเกณฑ์ตายของโจทย์ได้" },
        { q: "แก้ยังไง?", a: "วนหลับสั้น ๆ แล้วเทียบเวลาปัจจุบันกับเวลาเป้าหมายทุกรอบ — หลับทีละน้อยจนใกล้ถึงเวลาแล้วค่อยออก. แม่นกว่ามากโดยยังไม่กิน CPU เหมือนการวนเช็คเปล่า ๆ" },
        { q: "ทำไมไม่วนเช็คเวลาเฉย ๆ ไปเลย?", a: "กิน CPU เต็มแกนตลอดเวลาหลับ — พอมีนักปรัชญาหลายคนพร้อมกัน เครื่องจะช้าลงจนกระทบเวลาของทุกคน" },
      ]},
      { h: "📖 อ่านเพิ่มเติม (อยากเจาะทฤษฎีต่อ)" },
      { links: [
...
void displayStatus( void ) const;    // แค่แสดงผล → const`, lang: "cpp" },
      ...
      { p: "`_amount` คือเงินของ 'แต่ละบัญชี' (ต่างคนต่างมี) แต่ `_nbAccounts` คือ 'จำนวนบัญชีทั้งธนาคาร' — มีชุดเดียวที่ทุก object แชร์. static = ตัวแปรกลางของทั้ง class ไม่ใช่ของ object ตัวใดตัวหนึ่ง" },
...
→ getNbAccounts() คืน 3, getTotalAmount() คืน 1053 (ไม่ต้องมี object)`, cap: "หัวใจ ex02: _accountIndex มาจาก static counter ที่อ่านก่อนเพิ่ม — เลขจึงไม่ซ้ำและเรียงเอง", lang: "txt" },
      { note: "ลองลบดู: static member ต้อง 'นิยาม' นอก class ในไฟล์ .cpp ครั้งเดียว (`int Account::_nbAccounts = 0;`). ลองลบบรรทัดนี้ออก → linker error 'undefined reference to Account::_nbAccounts' เพราะ declaration ใน class แค่ 'ประกาศ' ไม่ได้ 'จองที่'" },
      { qa: [
      ...
      { p: "object **เกิด** → ctor ทำงาน (พิมพ์ `;created`), object **ตาย** → dtor ทำงาน (พิมพ์ `;closed`). ความท้าทายของ ex02 คือทำให้ลำดับ log ตรงกับไฟล์เป๊ะ — ซึ่งขึ้นกับ 'ลำดับเกิด/ตาย'" },
  ...
  • สมาชิกของ object เกิดตามลำดับ "การประกาศใน class" ไม่ใช่ลำดับใน init list`, cap: "สร้างเรียงหน้า–หลัง, ทำลายเรียงหลัง–หน้า (เหมือน stack ของจาน)", lang: "cpp" },
    ...
    // [ts] index:0;amount:42;closed    ← a ตายหลังสุด`, cap: "LIFO: ตัวที่เกิดทีหลังตายก่อน — เป็นเหตุผลที่ลำดับ closed กลับด้านกับ created", lang: "txt" },
      { note: "คิดตาม: ทำไมต้องเก็บ p_amount (เงินก่อนฝาก) ไว้ 'ก่อน' แก้ _amount ใน makeDeposit? เพราะ log ต้องพิมพ์ทั้งค่าเก่า (p_amount) และค่าใหม่ (_amount) — ถ้าแก้ก่อนเก็บ จะพิมพ์ค่าเก่าไม่ได้แล้ว ลำดับ 'อ่านเก่า → แก้ → พิมพ์ทั้งคู่' จึงสำคัญ" },
      { qa: [
      ...
      { p: "ใส่ `const` ท้าย method = ทำสัญญากับ compiler ว่า 'method นี้แค่อ่าน ไม่แก้ข้อมูล object'. มันไม่ใช่แค่ความสวยงาม — มันทำให้เรียก method นั้นกับ **const object** ได้ และให้ compiler จับ bug ถ้าเผลอแก้" },
...
→ const "แพร่" ผ่านระบบ: อยากให้ของ const ใช้งานได้ ต้องมาร์ค method อ่านอย่างเดียวเป็น const ให้ครบ`, cap: "const-correctness = ออกแบบให้ 'อะไรที่อ่านอย่างเดียว' มาร์ค const ครบ เพื่อให้ const object ใช้งานได้", lang: "txt" },
      { note: "ลองคอมไพล์ดู: เพิ่ม `const Account a(42); a.makeWithdrawal(5);` แล้วคอมไพล์ — จะ error เพราะ makeWithdrawal แก้ _amount (ไม่ใช่ const). เปลี่ยนเป็น `a.checkAmount()` (const) แล้วคอมไพล์ผ่าน — ยืนยันว่า const object เรียกได้แค่ const method" },
      { qa: [
      ...
      { p: "method เดียว (`makeDeposit`) ใช้ได้กับทุกบัญชี — มันรู้ได้ยังไงว่าตอนนี้ทำงานกับ a หรือ b? คำตอบคือ **`this`** pointer ซ่อนที่ชี้ไปยัง object ที่เรียก method นั้นอยู่" },
...
ภายใน method:
  ...
  → this ชี้ไป a → แก้ a._amount ; ถ้าเรียก b.makeDeposit → this ชี้ b`, cap: "this = ที่อยู่ของ object ที่เรียก — ทำให้โค้ดชุดเดียวทำงานกับ object ใดก็ได้", lang: "cpp" },
      ...
      { note: "สังเกต: ใน makeWithdrawal โค้ดเขียน `this->_amount` กับ `_amount` สลับกันได้ผลเหมือนกัน — เพราะ `_amount` เปล่า ๆ ก็คือ `this->_amount` โดยปริยาย. เขียน this-> ชัด ๆ ช่วยให้อ่านง่ายว่าแตะสมาชิกของ object" },
      { qa: [
...
/* ... execve ... */`, cap: "redirect > = dup2(file_fd, STDOUT) → สิ่งที่เขียนออก stdout ไปลงไฟล์แทน", lang: "c" },
  ...
  cmd2: dup2(fds[0], STDIN)   → อ่าน input จาก ท่อ`, cap: "ปิด fd ที่ไม่ใช้ให้หมด ไม่งั้น cmd2 จะรอ EOF ไม่มา= ค้าง", lang: "txt" },
      ...
      { p: "หั่นบรรทัดเป็นคำดูเหมือน 'แค่ตัดที่ช่องว่าง' แต่พอมี quote กฎเปลี่ยนทันที — `'a b'` คือ **คำเดียว** (ช่องว่างข้างในไม่ตัด). วิธีคิดที่ถูกคือมองเป็น **finite state machine**: เครื่องอยู่ในสถานะหนึ่ง อักขระที่เจอเปลี่ยนสถานะ/ตัดสินว่าจะตัดคำไหม" },
...
→ argv = ["echo", "a bcd"]  (ไม่ใช่ 3 คำ)`, cap: "หัวใจที่หลายคนพลาด: token ที่ติดกัน (ไม่มีช่องว่างคั่น) = คำเดียว แม้ quote คนละชนิด", lang: "txt" },
      ...
      { note: "ลองในเทอร์มินัล: `echo '$HOME'` ต้องได้ `$HOME` ตรง ๆ แต่ `echo \"$HOME\"` ต้องได้ path จริง — เพราะตัวแรกอยู่สถานะ S_QUOTE ที่ `$` ไม่ขยาย ส่วนตัวหลังอยู่ D_QUOTE ที่ `$` ขยาย" },
      { qa: [
      ...
      { p: "ตอน fork ลูกได้ 'สำเนาตาราง fd' ของพ่อมาทั้งใบ — fd เลขเดียวกันชี้ของเดียวกัน. การต่อ `a | b | c` คือการ 'เดินสาย' ว่า stdout ของ a → เข้า b, stdout ของ b → เข้า c ด้วย dup2 แล้วต้อง 'ปิดปลายที่ไม่ใช้' ให้เกลี้ยง" },
...
→ a จบ → pipe1 write ปิดหมด → b เห็น EOF ; b จบ → c เห็น EOF`, cap: "ปิดในพ่อทันทีทุกรอบ = กัน fork รอบถัดไปไม่ให้สืบทอด write-end เก่าติดไป", lang: "txt" },
      { note: "ลองคิดตาม: ถ้าพ่อ 'เลื่อน' การ close ไปทำทีเดียวตอนจบ (แทนที่จะปิดทันทีหลัง fork) → ตอน fork b พ่อยังถือ write-end 4 อยู่ → b ได้สำเนา fd 4 ติดไปด้วย → write-end ของ pipe1 ไม่มีวันเหลือ 0 → b รอ EOF ไม่มา = ค้าง. นี่คือเหตุผลที่ต้อง close ในพ่อ 'ทันที' ทุกรอบ" },
      { qa: [
      ...
      { p: "ปลายอ่านของ pipe จะได้สัญญาณ 'จบ' (EOF, read คืน 0) **ก็ต่อเมื่อปลายเขียนทุกสำเนาถูกปิดหมด** ไม่ใช่แค่ปลายเขียนหลัก. kernel นับว่ามีกี่ fd ที่ยังเปิด write-end อยู่ (reference count)" },
  ...
  cmd2 close(fds[1])              → 2
  พ่อ close(fds[1])               → 1   ← ถ้าลืมตรงนี้คือจุดที่ค้าง
...
ถ้าพ่อ "ลืม" close(fds[1]): refcount ค้างที่ 1 ตลอด
  → cmd2 อ่าน stdin รอ EOF ที่ไม่มีวันมา → ค้าง`, cap: "EOF = ฟังก์ชันของ refcount: ต้อง 0 เท่านั้น — ลืมปิดที่เดียว refcount ไม่ถึง 0 = ค้าง", lang: "txt" },
      ...
      { note: "ลองเอง: เขียน `cat | cat | cat` แล้วจงใจคอมเมนต์ close(fds[1]) ของพ่อออก 1 บรรทัด → รันแล้วค้าง. เปิด `ls -l /proc/<pid>/fd` ดูจะเห็น write-end ค้างเปิดอยู่ตามทฤษฎี refcount" },
      { qa: [
      ...
      { p: "กด Ctrl-C ตอนอยู่ที่ prompt ว่าง ๆ ต้อง 'ขึ้น prompt ใหม่บรรทัดถัดไป' (ไม่ออกจาก shell) แต่กด Ctrl-C ตอนคำสั่งกำลังรัน (เช่น `cat` รออยู่) ต้อง 'ฆ่าคำสั่งนั้น' แล้วกลับมา prompt. behavior ต่างกันตามบริบท" },
...
worked example:
  ...
  คำสั่ง segfault   → $? = 139  (128 + SIGSEGV 11)`, cap: "อ่าน exit status: WIFEXITED→WEXITSTATUS ; WIFSIGNALED→128+WTERMSIG", lang: "txt" },
      { note: "ลองในเทอร์มินัล: ใน bash จริงพิมพ์ `sleep 5` แล้วกด Ctrl-C ตามด้วย `echo $?` จะได้ 130. แล้วลอง `ls /nope; echo $?` ได้ 2 — minishell ต้องให้ค่าตรงกันเป๊ะ" },
      { qa: [
...
diffuse ∝ max(dot(N, L), 0)        # N=normal, L=ทิศไปหาแสง`, cap: "มุมตกตรง (dot สูง) = สว่าง; มุมเฉียง (dot ต่ำ) = มืด; ด้านหลังแสง (dot<0) = 0", lang: "txt" },
      ...
      { p: "ray คือเส้นตรง, sphere คือ 'จุดทั้งหมดที่ห่าง center = รัศมี r'. หา 'ชน' = หา t ที่ระยะจาก P(t) ถึง center = r พอดี — แทน P(t) ลงสมการ sphere ได้ **สมการกำลังสอง**" },
...
→ เลือกรากที่ 'เล็กสุดที่ยัง > EPSILON' (จุดเข้าที่อยู่หน้ากล้อง)
  normal ที่จุดชน = norm(P − center)`, cap: "disc < 0 = ไม่ชน; เลือก t เล็กสุดที่ >EPSILON = ผิวด้านที่กล้องเห็นก่อน", lang: "txt" },
      { note: "จำไว้: ไฟล์ subject ให้ 'เส้นผ่าศูนย์กลาง' (diameter) ไม่ใช่รัศมี — ต้อง **หาร 2** ก่อนใช้เป็น r. นี่คือบั๊กอันดับ 1: ลืม /2 แล้ว sphere ใหญ่เกิน 2 เท่า" },
      { qa: [
      ...
      { p: "ผิวที่หันเข้าหาแสงตรง ๆ สว่างสุด, หันเฉียงมืดลง, หันหนีแสงมืดสนิท. วัดด้วย **dot(N, L)** — มุมระหว่าง normal กับทิศไปหาแสง (Lambert's cosine law)" },
  ...
  return clamp(color, 0, 1)`, cap: "⊙ = คูณทีละช่อง (R×R,G×G,B×B); dot(N,L) = cos(มุม) = ความสว่างตามมุมตกกระทบ", lang: "txt" },
      ...
      { note: "กับดักครึ่งหนึ่งของ 'จุดดำ': หลังคำนวณ N ต้อง **พลิก normal เข้าหากล้อง** ก่อนใช้: `if dot(N, D) > 0: N = −N`. ไม่งั้นผิวด้านในวัตถุ/cylinder จะคำนวณแสงกลับด้าน มืดผิด" },
      { qa: [
      ...
      { p: "จุดบนผิวโดนแสงไหม → ยิง 'shadow ray' จากจุดนั้นไปหาแสง ถ้าชนวัตถุอื่นก่อนถึงแสง = อยู่ในเงา. แต่มีปัญหาคลาสสิก: จุดนั้น **บังเงาตัวเอง**" },
...
ปัญหาถ้าไม่เลื่อน (origin = P เป๊ะ):
  ...
  → ทุกจุดดำหมด = "shadow acne" (จุดดำกระจายเป็นเม็ด)`, cap: "เลื่อน origin ออกตาม normal นิดนึง (EPSILON ~1e-3) → ไม่ชนตัวเอง", lang: "txt" },
      { note: "ลองเอง: ตั้ง EPSILON = 0 แล้ว render — จะเห็นจุดดำกระจายทั่วผิว (acne). เพิ่ม EPSILON เป็น 1e-4..1e-3 แล้วจุดดำหายทันที. และต้องเช็ค `hit.t < dist` ไม่งั้นวัตถุ 'หลังแสง' จะถูกนับเป็นตัวบังผิด" },
      { qa: [
      ...
      { p: "กล้องมีตำแหน่ง + ทิศมอง + FOV (มุมมองแนวนอน). ต้องสร้าง 'จอเสมือน' หน้ากล้อง แล้วยิง ray จากกล้องผ่านแต่ละ pixel. ขั้นแรก: สร้าง **orthonormal basis** (3 แกนตั้งฉากกัน)" },
...
ต่อ pixel (x,y) — มี +0.5 (กลาง pixel), พลิก y (จอ y ลง แต่ภาพ y ขึ้น):
  ...
  dir = norm(forward + px·right + py·up)`, cap: "basis = 3 แกนตั้งฉาก; pixel แมพเป็นออฟเซ็ตบน right/up แล้วบวก forward = ทิศ ray", lang: "txt" },
      ...
      { note: "ลองจัดฉากดู: ตั้งกล้อง C ที่ (0,0,−30) มอง +z, วาง sphere ที่ (0,0,12) — ต้องเห็นวงกลมกลางจอ. ถ้าจอดำสนิท = กล้องมองผิดทาง (วัตถุอยู่นอกเฟรม) — จัดฉากให้วัตถุอยู่หน้ากล้องก่อน" },
      { qa: [
...
color [0,1] → color_to_int → image buffer → mlx_put_image`, cap: "ไฟล์ฉาก → struct → ray ต่อ pixel → สี → ภาพ", lang: "txt" },
    ...
    y = 0;
    while (y < HEIGHT)                            // norm ห้าม for → while
    {
        x = 0;
        while (x < WIDTH)
        {
...
ray_dir  = dir + plane · camera_x  # คอลัมน์กลาง = dir ตรง ๆ`, cap: "กวาด camera_x จาก −1→+1 = กวาด ray จากขอบซ้าย→ขวาของมุมมอง", lang: "txt" },
      ...
      { p: "ผู้เล่นยืนบน grid 2D มอง 'ไปข้างหน้า' (dir). จะเห็นมุมกว้างได้ต้องมี 'จอเสมือน' วางตั้งฉากหน้าผู้เล่น = เวกเตอร์ **plane**. ทุกคอลัมน์จอ = 1 จุดบน plane → ยิง ray ผ่านจุดนั้น" },
...
ทิศเริ่มต้นจากตัวอักษรผู้เล่น (boolean arithmetic, เลี่ยง ?: ตาม norm):
  ...
  → plane ตั้งฉาก dir เสมอ (หมุน dir 90°)`, cap: "plane ยาวขึ้น = FOV กว้างขึ้น (มากไปจะ fisheye); plane ⊥ dir เสมอ", lang: "txt" },
      { note: "ลองปรับดู: ตั้ง |plane| = 1.0 (FOV 90°) แล้ว 0.3 (FOV ~33°) — ดูภาพกว้าง/แคบเปลี่ยน. ตอนหมุนตัว ต้องหมุน **ทั้ง dir และ plane** ด้วย rotation matrix เดียวกัน ไม่งั้นกล้องเพี้ยน" },
      { qa: [
      ...
      { p: "แทนที่จะคืบ ray ทีละนิด (ช้า/พลาด) DDA กระโดดไปที่ **เส้น grid ถัดไป** ทันที (แนวตั้งหรือแนวนอนอันไหนใกล้กว่า) แล้วเช็คช่องนั้นเป็นผนังไหม — รับประกันไม่ข้ามผนัง" },
...
วน DDA:
  while (ช่องปัจจุบันไม่ใช่ผนัง):
    ...
    if (map เป็น '1' / space / นอกขอบ): break   # เจอผนัง`, cap: "เลือกเส้น grid ที่ใกล้กว่าเดินไปทีละช่อง → เร็วและไม่มีทางทะลุผนัง", lang: "txt" },
      ...
      { note: "จำไว้: ต้อง treat **space และนอกขอบ = ผนัง** ด้วย ไม่งั้นผู้เล่นยิง ray ทะลุออกนอกแผนที่แล้ว DDA วนไม่จบ (segfault/ค้าง). guard delta ด้วย 1e30 เมื่อ ray_dir = 0 (ray ขนานแกน)" },
      { qa: [
      ...
      { p: "ถ้าวัดระยะจากผู้เล่นถึงผนังแบบ Euclidean (ระยะตรงจริง) ผนังเรียบจะดู **โป่งกลางจอ** เพราะ ray ที่ขอบจอวิ่งเฉียงไกลกว่า ray กลางจอ → ผนังเดียวกันดูใกล้ตรงกลาง ไกลที่ขอบ = บิดเบี้ยว" },
...
แทนที่จะ √(dx²+dy²) ใช้ค่าที่ DDA มีอยู่แล้ว:
  ...
  line_h = WIN_H / perp`, cap: "perp = ระยะตั้งฉากกับระนาบกล้อง → ผนังเรียบดูเรียบ (ลบ fisheye)", lang: "txt" },
      ...
      { note: "ลองสลับดู: ใช้ระยะ Euclidean (√) แทน perp แล้ว render — ผนังจะโค้งโป่งกลางจอเหมือนมองผ่านเลนส์ตาปลา. เปลี่ยนกลับเป็น perp แล้วผนังตรงทันที" },
      { qa: [
      ...
      { p: "รู้ว่าชนผนังแล้ว ยังต้องตอบ 2 ข้อ: (1) ผนัง **ด้านไหน** (เหนือ/ใต้/ออก/ตก → texture ไหน) (2) ชน **ตรงไหน** บนผนัง → คอลัมน์ไหนของ texture" },
...
พลิก tex_x 2 ใน 4 กรณี (ไม่งั้น texture กลับด้าน):
  ...
  // เหนือ start = สีเพดาน, ใต้ end = สีพื้น`, cap: "ผนังใกล้ (line_h สูง) → step เล็ก = texture ยืด; ไกล → step ใหญ่ = บีบ", lang: "txt" },
      { note: "ลองข้ามขั้นดู: ไม่ต้องพลิก tex_x — เดินรอบห้องจะเห็น texture บางด้าน 'กลับซ้ายขวา' (เช่นตัวหนังสือบนผนังกลับด้าน). และต้อง `& (TEX−1)` clamp ไม่งั้นผนังที่ชิดตัวมาก (line_h > จอ) อ่าน texture เกินขอบ → segfault" },
      { qa: [
   ...
   └─ mlx_put_image_to_window (ทั้งเฟรมทีเดียว)`, cap: "parse→validate (headless) เกิดก่อน mlx → error path ไม่ leak X11", lang: "txt" },
    ...
    implementation: [
      ...
      { table: { head: ["อาการ", "สาเหตุ", "แก้"], rows: [
        ...
        ["norminette ไม่ผ่านทั้งที่โค้ดดูถูกต้อง", "ไฟล์มี CRLF (\\r) / header ยาว 81 คอลัมน์", "sed ลบ \\r + ก๊อป header จากไฟล์ที่ผ่านแล้ว"],
      ]}},
