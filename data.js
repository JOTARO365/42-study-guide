...
U+0080..U+07FF      2 bytes      110xxxxx 10xxxxxx  (á, ñ, ก-ฮ)
U+0800..U+FFFF      3 bytes      1110xxxx 10xxxxxx 10xxxxxx  (中文, 日本語)
...
วงจรรอ (เมื่อทุกคนหยิบซ้ายก่อนพร้อมกัน):
  ...
  P4 ถือ fork4 รอ fork0 (P0 ถือ)  → ครบวง = ค้าง`, cap: "ทุกคน 'หยิบทิศเดียวกัน' จึงเกิดวงปิด — หัวใจของ deadlock อยู่ที่ความสมมาตรนี้", lang: "txt" },
      { p: "**พิสูจน์ว่าทำไม even/odd ตัดวงจรได้:** ให้คน id คู่หยิบ **ซ้ายก่อนขวา** ส่วนคน id คี่หยิบ **ขวาก่อนซ้าย**. การจะเกิด circular wait ทุกคนต้องร้องขอทรัพยากรใน 'ทิศหมุนเดียวกัน' รอบโต๊ะ — แต่พอคู่กับคี่ขอคนละทิศ ความสมมาตรถูกทำลาย จะมีอย่างน้อย 1 จุดที่สองคนเอื้อมหยิบส้อม 'อันเดียวกัน' จากคนละฝั่ง → คนหนึ่งได้ทั้งคู่ อีกคนไม่ได้เลย → ไม่มีวงปิด → ไม่มี deadlock ▮" },
      { code: String.raw`ตัวอย่าง 2 คน 2 ส้อม (n=2):
  P0 (คู่): wait fork0(ซ้าย) → fork1(ขวา)
  P1 (คี่): wait fork0(ขวา) → fork1(ซ้าย)   ← ขอ fork0 ก่อนเหมือนกัน!
  → ทั้งคู่แย่ง fork0 ก่อน: ใครได้ก่อนได้ไปเลย
  → คนนั้นหยิบ fork1 ต่อได้ → กิน → วาง → อีกคนค่อยกิน
  (ไม่เกิดสภาพ 'ต่างคนต่างถือ 1 อันแล้วรอ')`, cap: "การจัดลำดับทรัพยากร (resource ordering) = วิธีคลาสสิกของ Dijkstra ในการตัด circular wait", lang: "txt" },
      { note: "ลองพิสูจน์เอง: วาด 3 คน 3 ส้อม ให้ทุกคนหยิบซ้ายก่อน — จะเห็นวงปิด P0→P1→P2→P0. จากนั้นเปลี่ยนให้คนคี่ (P1) หยิบขวาก่อน แล้วไล่ดูว่าวงขาดตรงไหน" },
      { qa: [
