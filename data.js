...
/* ===================== PUSH_SWAP ===================== */
{
  ...
  sections: {
    principle: [
      { p: "เกริ่นก่อน: push_swap ไม่ใช่แค่ 'เขียนโปรแกรมเรียงเลข' — มันคือปริศนาที่บังคับให้เราเรียงข้อมูลผ่านชุดคำสั่งเล็ก ๆ ที่ตายตัว ให้ถูกที่สุดเท่าที่จะทำได้ นั่นคือเหตุผลที่มันฝึกการคิดเชิง algorithm ได้จริง" },
      { h: "โจทย์คืออะไร" },
      ...
      { h: "เกณฑ์คะแนน (subject v1.1 — 3 ขั้น)" },
      { table: { head: ["ขั้น", "100 ตัว", "500 ตัว"], rows: [
        ["excellent", "< 700 ops", "< 5500 ops"],
        ["good", "< 1500", "< 8000"],
        ["ผ่านขั้นต่ำ", "< 2000", "< 12000"],
      ]}},
      { note: "วัดจาก strategy ปริยาย (`--adaptive`) เท่านั้น — บังคับ flag แล้ววัดไม่นับ. bonus จะถูกตรวจก็ต่อเมื่อ mandatory ผ่าน benchmark ครบทุกข้อ" },
      ...
      { h: "subject v1.1 เปลี่ยนโจทย์ไปมาก — อ่านตรงนี้ก่อน" },
      { ul: [
        "เป็น **group activity 2 คนเป๊ะ ๆ** — README บรรทัดแรกต้องมี login ทั้งสองคน",
        "ต้องวัด **disorder** (ความไม่เรียงของ input) ก่อนขยับ stack ครั้งแรก",
        "binary เดียวต้องฝัง **4 strategy** และเลือกได้ตอนรันด้วย `--simple` `--medium` `--complex` `--adaptive`",
        "ต้องมี `--bench` รายงาน disorder / ชื่อ strategy / complexity class / จำนวน op แต่ละชนิด ออกทาง **stderr**",
        "**complexity class ที่เคลมต้องจริงในหน่วย operation** ไม่ใช่ Big-O ของ algorithm แบบ array — นี่คือสิ่งที่ผู้ตรวจจี้",
      ]},
      { h: "disorder — ตัวเลขเดียวที่ตัดสินว่าจะใช้ algorithm ไหน" },
      { code: String.raw`disorder = จำนวนคู่ที่สลับกัน / จำนวนคู่ทั้งหมด
         = |{(i,j) : i < j และ a[i] > a[j]}| / (n(n-1)/2)

0.0  เรียงอยู่แล้ว
0.5  ค่าคาดหวังของ permutation สุ่ม   <-- จุดตัดสำคัญ
1.0  กลับด้านสมบูรณ์`, cap: "นับ inversion หารด้วยจำนวนคู่ทั้งหมด — ต้องวัดก่อน emit op ตัวแรก", lang: "text" },
      { p: "ที่ 0.5 ไม่ใช่เลขสุ่ม: **permutation สุ่มมี disorder คาดหวัง 0.5 พอดี** และที่ n=100 การกระจายรอบค่านี้แคบมาก เกณฑ์ 0.5 จึงเป็นเส้นแบ่งระหว่าง 'input นี้แยกไม่ออกจากสุ่ม' กับ 'ยังมีโครงสร้างเหลือให้ใช้ประโยชน์'" },
      { h: "4 strategy ที่ต้องอยู่ใน binary เดียว" },
      { table: { head: ["flag", "วิธี", "class (หน่วย operation)", "ขอบเขต"], rows: [
        ["--simple", "selection sort — ดันตัวน้อยสุดลง B ทีละตัว", "O(n²)", "n รอบ × หมุนไม่เกิน n/2 + push 2n"],
        ["--medium", "chunk sort — แบ่งเป็น ~√n ก้อน ก้อนละ ~√n ค่า", "O(n·√n)", "หมุนไม่เกิน n ต่อก้อน × √n ก้อน"],
        ["--complex", "binary radix (LSD) — แยกทีละ bit", "O(n log n)", "≤ 2n ต่อรอบ × ceil(log₂ n) รอบ"],
        ["--adaptive", "เลือกตาม disorder (ปริยาย)", "ตามช่วงที่ตกลงไป", "ดูตารางในหัวข้อ implementation"],
      ]}},
      { note: "radix เป็นตัวเดียวที่ **ราคาไม่ขึ้นกับการจัดเรียงของ input** — เพราะแบบนั้นมันจึงเป็นตัวเลือกที่ถูกต้องเมื่อ input ไม่เหลือโครงสร้างให้ใช้แล้ว" },
...
pa(ps);                      // ดันเข้า A`, lang: "c" },
      { h: "7) disorder.c — วัดความไม่เรียงก่อนขยับ (v1.1)" },
      { code: String.raw`double compute_disorder(t_node *a) {
    long mistakes = 0, pairs = 0;
    for i in a:                      // เขียนจริงด้วย while ตาม norm
        for j in a after i:
            pairs++;
            if (i->val > j->val) mistakes++;
    if (pairs == 0) return (0.0);
    return ((double)mistakes / (double)pairs);
}`, cap: "O(n²) ตรงตาม pseudocode ของ subject — รันครั้งเดียวก่อน sort จึงไม่อยู่บน hot path", lang: "c" },
      { p: "n=500 คือ 124,750 คู่ เทียบกับ operation หลักพันที่จะตามมา — เปลี่ยนเป็น merge-count O(n log n) ได้แต่ไม่ได้เวลาคืนมาจริง แลกกับโค้ดที่อ่านรอบเดียวไม่เข้าใจ ใช้ `long` นับคู่เผื่อ n โตกว่านี้" },
      { h: "8) 4 strategy — ไฟล์ละหนึ่งวิธี" },
      { code: String.raw`void sort_complex(t_ps *ps) {          // binary radix (LSD)
    bits = จำนวน bit ของ (size - 1);   // ทำได้เพราะ normalize แล้ว
    while (i < bits && !is_sorted(ps->a)) {
        j = 0;
        while (j++ < size) {
            if ((ps->a->idx >> i) & 1) ra(ps);   // bit 1 คาไว้ใน A
            else                       pb(ps);   // bit 0 ลง B
        }
        while (ps->b) pa(ps);          // ดึงกลับ = stable partition 1 bit
        i++;
    }
}`, cap: "แต่ละรอบคือการแบ่งแบบ stable ตาม bit เดียว ครบ ceil(log₂ n) รอบแล้วเรียงพอดี", lang: "c" },
      { p: "chunk sort (`--medium`) มีกับดักหนึ่งที่ทำให้ค้าง: **ต้องเลื่อนขอบก้อนโดยนับจำนวนที่ push แล้ว** ไม่ใช่เงื่อนไขอื่น. ถ้าเลื่อนผิดจังหวะจะเกิดสภาพที่ไม่มีตัวไหนใน A เข้าเงื่อนไขเลย แล้ว `else ra(ps)` จะหมุนวนไม่รู้จบ. อีกจุดคือ `rb` ตัวที่อยู่ครึ่งล่างของก้อนทันทีหลัง push เพื่อดันของเล็กลงก้นกอง ทำให้ B เกือบเรียงจากมากไปน้อย รอบดึงกลับจึงหมุนไม่เกินความกว้างของก้อนเดียว" },
      { h: "9) strategy.c — เลือกวิธีจาก disorder" },
      { code: String.raw`size <= 5        -> sort_small        (hardcode, ขอบบน O(n^2))
disorder < 0.2   -> greedy insertion  (O(n^2))
disorder < 0.5   -> chunk sort        (O(n*sqrt(n)))
disorder >= 0.5  -> binary radix      (O(n log n))`, cap: "subject กำหนดเพดาน class ของแต่ละช่วง ที่เหลือคือเราต้องอธิบายว่าเลือกวิธีไหนในเพดานนั้น และ threshold มาจากไหน", lang: "text" },
      { note: "ราคาที่ต้องจ่ายเพื่อพูดความจริง: input สุ่มตกที่ disorder ~0.5 พอดี ทำให้ greedy insertion (เร็วสุด ~550-620 ops ที่ n=100) ถูกห้ามใช้เพราะเป็น O(n²) ต้องใช้ radix ที่ ~1050 แทน = จ่ายเพิ่มราว 400 ops และตกจากขั้น excellent มาเป็น good. เขียนตัวเลขนี้ลง README ตรง ๆ — การ route input สุ่มไป greedy แล้วติดป้าย O(n log n) ได้คะแนนดีกว่าแต่เป็นการเคลมเท็จ ซึ่งเป็นสิ่งเดียวที่ subject เวอร์ชันนี้ออกแบบมาจับ" },
      { h: "10) bench.c — รายงานออก stderr" },
      { code: String.raw`$ ./push_swap --bench 4 67 3 87 23 > /dev/null
[bench] disorder: 40.00%
[bench] strategy: simple (selection sort) O(n^2)
[bench] operations: 9
[bench] sa=0 sb=0 ss=0 pa=2 pb=2 ra=4 rb=0 rr=0 rra=1 rrb=0 rrr=0`, cap: "stdout เหลือแต่ op ล้วน ๆ จึงยัง pipe เข้า checker ได้พร้อมกับ 2> bench.txt", lang: "text" },
      { p: "`ft_printf` เขียนลง fd 1 อย่างเดียว — จะออก fd 2 ต้องใช้ `ft_putstr_fd` / `ft_putnbr_fd` ของ libft คู่กับ `write(2, ...)`. เปอร์เซ็นต์ทศนิยม 2 ตำแหน่งทำได้โดยไม่ต้องพิมพ์ float: คูณ 10000 ปัดครั้งเดียวเป็น `long` แล้วพิมพ์ผลหาร จุด และเศษ — อย่าลืมเติม 0 นำหน้าเมื่อเศษ < 10 ไม่งั้น 40.05% จะกลายเป็น 40.5%" },
    ],
    tricks: [
      ...
      { h: "ทริค 9: นับ operation ที่จุดเดียวด้วย emit()" },
      { p: "ให้ทุก op วิ่งผ่าน `emit(ps, OP_XX)` ตัวเดียวที่บวก `ps->cnt[op]` แล้วค่อย write. ได้สองอย่าง: ตัวเลขใน `--bench` เป็นคำสั่งที่ **พิมพ์ออกไปจริง** ไม่ใช่ค่าประมาณจาก algorithm และโหมด silent ของ checker ที่ปิดการพิมพ์ก็ยังนับถูก. ชื่อ op ให้คืนจาก if-chain ไม่ใช่ `static const` array — subject ห้าม global variable" },
      { h: "ทริค 10: flag ต้องขึ้นต้นด้วย -- เท่านั้น" },
      { p: "กฎข้อเดียวนี้ทำให้ `-5` ยังเป็นจำนวนลบที่ถูกต้อง ส่วน `--simple` เป็น selector และ flag วางตรงไหนของ argument ก็ได้ ไม่ต้องบังคับลำดับ. flag ที่ไม่รู้จัก → `Error`. ส่วน `checker` ไม่รับ flag เลย จึงนับจำนวน flag ตอน parse แล้ว reject ถ้าไม่เป็นศูนย์" },
      { h: "ทริค 11: ผูก node เข้า ps->a ทันที ไม่พักไว้ในตัวแปร local" },
      { p: "`error_exit` free `ps` — อะไรที่ยังค้างใน local head จะรั่ว. นี่เป็น leak จริงที่โปรเจกต์นี้ติดบ่อยเพราะ valgrind จะเห็นก็ต่อเมื่อทดสอบด้วย input ที่ **ผิด** ซึ่ง tester ส่วนใหญ่ข้าม" },
    ],
    eval: [
      { qa: [
...
./push_swap 1 2 2        # Error (ซ้ำ)`, lang: "bash" },
      { h: "คำสั่งทดสอบ v1.1 (4 strategy + bench)" },
      { code: String.raw`# ทุก strategy ต้องได้ OK และต้องวัด ops ที่แย่ที่สุดจากหลายรอบ
...
# bench ออก stderr จึง pipe เข้า checker พร้อมกันได้
./push_swap --bench $ARG 2> bench.txt | ./checker_linux $ARG

# error / no-output
./push_swap --nope 1 2      # Error
./push_swap --simple 3 2 3  # Error (เลขซ้ำ)
./push_swap 1 2 3           # ไม่พิมพ์อะไร`, cap: "input สุ่มจะตกที่ disorder ~0.5 เสมอ", lang: "bash" },
      { note: "input สุ่มทดสอบได้แค่ช่วงเดียว. ต้องสร้าง input อีกสองช่วงเองแล้วยืนยันจาก `--bench` ว่า strategy ที่ตั้งใจถูกเรียกจริง: เรียงแล้วสลับคู่ติดกันไม่กี่คู่ ได้ disorder < 0.2, และสุ่มสับ 40-80 ตำแหน่งจาก 100 ได้ 0.2-0.5" },
    ],
  },
},
