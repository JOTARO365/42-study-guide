...
pa(ps);                      // push into A`, lang: "c" },
    ...
    tricks: [
      ...
      { p: "When A and B both need to turn the same way, use `rr` instead of `ra` + `rb` — the cost of that move becomes `max(|ca|, |cb|)` rather than `|ca| + |cb|`. **This is the single biggest lever for getting under 700 / 5500**" },
      { h: "Trick 2: keep the direction in the sign of the cost" },
      { p: "`+3` means three `ra`, `-2` means two `rra` — no separate direction variable, and checking 'do they turn the same way?' becomes a sign comparison" },
      { h: "Trick 3: normalise into ranks before anything else" },
      { p: "Convert to 0..n-1 and the whole algorithm works on `idx` — overflow stops being a concern and target positions compare directly against `idx`" },
      { h: "Trick 4: A is circularly sorted — one final rotation finishes it" },
      { p: "While inserting, A doesn't have to run smallest-to-largest from the top — it only has to be **sorted as a circle**, and one rotation at the end brings the smallest to the top. That relaxed condition is much cheaper to maintain, so inserts get cheaper too" },
      { h: "Trick 5: the silent flag lets one codebase drive the checker" },
...
run_child(prev, fds[1], cmds[n-1], envp);  // last cmd -> outfile`, cap: "extends from 2 to N commands by rolling the pipe forward", lang: "c" },
    ...
    tricks: [
      ...
      { h: "Trick 5: the rolling pipe for the bonus" },
...
mlx_loop(game.mlx);`, cap: "main.c: bind events then enter the loop", lang: "c" },
    ...
    tricks: [
      ...
      { h: "Trick 4: camera = player − half a screen, then clamp" },
      { p: "Those two steps immediately support maps larger than the window, and the clamp is what stops the camera drifting off the edge of the map" },
      { h: "Trick 5: event 17 is the window's X button" },
      { p: "`DestroyNotify` has to be hooked separately from ESC — skip it and clicking the cross leaves a stray process or crashes, which the evaluator will absolutely try" },
      { h: "Trick 6: validate the map fully before opening a window" },
      { p: "A `.ber` extension · rectangular · walls all around · exactly one `P`, at least one `E` and one `C` · no stray characters · every `C` and the `E` reachable. Opening a window and only then discovering the map is broken makes cleanup harder than it needs to be" },
      { h: "Trick 7: release MiniLibX resources in order" },
      { p: "`mlx_destroy_image` for every image → `mlx_destroy_window` → `mlx_destroy_display` → `free(mlx)`, then free the map. Out of order, valgrind reports invalid reads" },
      { h: "Trick 8: count a move only when one actually happens" },
      { p: "Walking into a wall doesn't count. The subject asks for the counter in the terminal; drawing it into the window is a bonus" },
    ],
    eval: [
      { qa: [
...
return ((r << 16) | (g << 8) | b);`, cap: "a polynomial palette gives a smooth gradient; cast to int before shifting — shifting a double doesn't compile", lang: "c" },
    ...
    tricks: [
      ...
      { h: "Trick 3: Mandelbrot and Julia share one loop" },
      { p: "They differ only in the starting values — Mandelbrot: `z = 0`, `c = the pixel`; Julia: `z = the pixel`, `c = a constant`. Write one function and pass those two in" },
      { h: "Trick 4: zoom around the cursor, not the centre of the window" },
      ...
      { h: "Trick 8: colour by iteration count, not by in/out" },
      { p: "Points that escape quickly and slowly get different colours — that is what makes the structure around the boundary visible. Two colours alone gives a flat blob with no detail at all" },
    ],
    eval: [
      { qa: [
