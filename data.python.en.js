...
# must still say "still plugged in" — sys.prefix decides`,
  ...
  amaze: {
    principle: [
      { h: "What a perfect maze is" },
      { p: "A **perfect maze** is one where every cell is reachable and there is **exactly one path** between any two cells — no loops, no cut-off regions." },
      { note: "**That definition is precisely a spanning tree of the grid graph.** Cells are nodes, removed walls are edges. A `w × h` grid has `w·h` cells, so a spanning tree has exactly `w·h − 1` edges." },
      { p: "The consequence is a **test you can actually run**: count the walls removed — it must equal `w·h − 1` — and run one BFS from a corner, which must reach every cell. Together those two prove a perfect maze without looking at a picture." },
      { h: "What gets submitted" },
      { table: { head: ["File", "Its job"], rows: [
        ["`a_maze_ing.py`", "the main program: arguments and the config file"],
        ["`render.py`", "turns the grid into text"],
        ["`mazegen/`", "the algorithm package, with its `__init__.py`"],
        ["`config.txt`", "defaults the program can read"],
        ["`Makefile`", "`install`, `run`, `debug`, `clean`, `lint`"],
        ["`pyproject.toml`", "makes `mazegen` an installable wheel"]
      ]}},
      { p: "The Python series' shared rules all apply here — flake8 at 79 columns, full type hints, no tracebacks. See the **Python Modules 00–04** page for the detail." }
    ],

    theory: [
      { h: "Walls as bits" },
      { p: "Each cell has four walls, and they fit in a single number by giving each side one bit:" },
      { table: { head: ["Direction", "Bit", "Value"], rows: [
...
# knocking a wall down between two cells — both sides, always
grid[y][x] &= ~direction
grid[ny][nx] &= ~OPPOSITE[direction]`,
        cap: "One int per cell", lang: "python" },
      { note: "**A wall must always be removed from both sides.** Remove one and cell A says it can reach B while B says it cannot come back; the renderer then draws a maze that contradicts itself. The test that catches this walks every adjacent pair and asserts the two wall states agree." },
      { h: "Why the DFS has to be iterative" },
      { p: "The standard algorithm is the **recursive backtracker**: walk randomly deeper, knocking down each wall you pass, and back up at a dead end. The problem is that written recursively the stack depth equals the longest path — a 100×100 grid can hit `RecursionError`." },
...
while stack:
    ...
    visited.add((nx, ny))
    stack.append((nx, ny))`,
        cap: "An explicit stack instead of Python's call stack", lang: "python" },
      { p: "Same structure, same output, but the depth now lives in a list bounded by memory rather than by a 1000-frame limit." },
      { h: "Randomness you can reproduce" },
      { p: "Use a `random.Random(seed)` object of your own, **not** the module-level `random.seed()` — the latter changes shared state for the whole program, so any other code that draws random numbers changes behaviour too." },
      { code: String.raw`rng = random.Random(seed)          # our own state
# same seed + same size = byte-identical maze, every time`,
        cap: "The seed is what makes the tests trustworthy", lang: "python" }
    ],

    architecture: [
      { h: "Keep the algorithm away from the display" },
      { code: String.raw`a_maze_ing.py          # arguments, config, calls both halves
render.py              # grid -> text (knows no algorithm)
mazegen/
|-- __init__.py        # exposes generate() and the direction constants
'-- generator.py       # the iterative DFS (knows no drawing)
config.txt
pyproject.toml
Makefile`,
        cap: "The generator never sees a character; the renderer never sees a bit", lang: "text" },
      { p: "This split is not decoration — it is what makes it possible to **test the algorithm without parsing text**. The tests call `generate()` and inspect the graph directly." },
...
python3 -c "import mazegen; print(mazegen.generate(3, 3, seed=1))"`,
        cap: "Proof that it installs, not merely that it imports from the same folder", lang: "bash" },
      { note: "The test that matters is **installing it and running from a different directory**. If the import still works, the package is real and not relying on happening to be in the current directory." }
    ],

    dataflow: [
      { h: "From bits to a picture" },
      { p: "Drawing follows one rule: each cell draws its own **top** and **left** edges, and the row and the grid are closed off with a final right and bottom edge. Nothing is ever drawn twice." },
      { code: String.raw`+---+---+---+
|           |     cell (0,0) has no E wall, so no | divides it
+---+   +---+
|   |       |
+   +---+   +
|           |
+---+---+---+`,
        cap: "What render produces", lang: "text" },
      { h: "The data's path through the program" },
      { table: { head: ["Stage", "Input", "Output"], rows: [
        ["read config", "`config.txt` + arguments", "`width`, `height`, `seed`"],
        ["validate", "numbers that could be anything", "usable values, or a readable error"],
        ["generate", "`w`, `h`, `seed`", "`list[list[int]]` — the wall bits"],
        ["render", "the grid", "a multi-line string"]
      ]}},
      { p: "**Arguments always beat the config file** — the same convention as environment variables beating `.env` in module 08: the more specific source, closer to the user, wins." },
      { h: "Inputs that must be refused" },
      { table: { head: ["Input", "What must happen"], rows: [
        ["`0` or negative", "an error message, not an empty grid and not a traceback"],
        ["`1 × 1`", "valid — one cell, zero walls removed (`1·1 − 1`)"],
        ["`1 × 50`", "valid — a single-row maze is still a spanning tree"],
        ["not a number", "an error naming the offending value"],
        ["very large, e.g. `200 × 200`", "must not raise `RecursionError` — this is why the DFS is iterative"],
        ["`config.txt` missing or malformed", "fall back to defaults and say so, rather than failing"]
      ]}}
    ],
...
def reachable(grid: list[list[int]]) -> int:
    """BFS from (0,0); count the cells it reaches."""
    seen = {(0, 0)}
    queue = [(0, 0)]
    while queue:
        x, y = queue.pop()
...
for width, height in ((1, 1), (1, 9), (9, 1), (13, 7), (40, 40)):
    ...
    assert edges_removed(grid) == width * height - 1   # it is a tree
    assert reachable(grid) == width * height           # and it is connected`,
        cap: "Those last two lines are the definition of a perfect maze", lang: "python" },
      { p: "A spanning tree must be both acyclic and connected. The edge count of `n − 1` gives the first, and a BFS reaching everything gives the second. With both, nothing else needs checking." },
      { h: "The three test tiers for this project" },
      { table: { head: ["Tier", "What it checks"], rows: [
        ["**NORMAL**", "ordinary sizes generate, the drawing has the right number of lines, every `Makefile` target works"],
        ["**EXTREME**", "`1×1`, `1×n`, `n×1`, zero and negative values, a missing `config.txt`, non-numeric values, arguments overriding config"],
        ["**HARDCORE**", "the spanning-tree property at every size, walls agreeing on both sides, the same seed reproducing, different seeds differing, `200×200` not hitting the recursion limit, the installed wheel importing from another directory, flake8 and `mypy --strict`"]
      ]}},
      { code: String.raw`# The same seed must reproduce, and a different one must not
assert generate(20, 20, seed=42) == generate(20, 20, seed=42)
assert generate(20, 20, seed=42) != generate(20, 20, seed=43)

# Walls must agree on both sides, for every adjacent pair
for y in range(height):
    for x in range(width - 1):
        assert bool(grid[y][x] & E) == bool(grid[y][x + 1] & W)`,
        cap: "The two tests that catch the most real bugs", lang: "python" }
    ],
    ...
    tricks: [
      ...
      { table: { head: ["Symptom", "Cause"], rows: [
        ...
        ["The maze has loops", "a wall was removed towards an already-visited cell; check visited before choosing a neighbour"],
        ["Some cells are unreachable", "fewer than `n−1` edges, usually from popping the stack too early"],
        ["The drawing contradicts itself", "a wall was removed on one side only"],
        ["Every row is identical", "`grid = [[15] ‘ w] ’ h` — every row is the same list; use a comprehension"],
        ["The same seed gives different mazes", "module-level `random.seed()`, which other code can disturb; use your own `random.Random(seed)`"],
      ...
      { ul: [
        ...
        "Keep `visited` as a set of tuples rather than scanning the grid — set membership is O(1)",
        "Write `OPPOSITE` as a dict instead of computing it with bit arithmetic — easier to read and harder to get wrong",
        "Have `generate()` return a plain grid, not a class — the tests can then compare two mazes with `==`",
        "**Validate input at one boundary**, where arguments and config are read, and the rest of the program needs no checks"
      ]}
    ],

    eval: [
      { qa: [
        { q: "What is a perfect maze, and what has it to do with a spanning tree?", a: "A perfect maze is one where every cell is reachable and exactly one path connects any two cells, which is the definition of a tree over the grid graph. Cells are nodes and removed walls are edges, so a grid of n cells must have exactly n−1 edges, and one BFS must reach every cell." },
        { q: "Why an iterative DFS rather than a recursive one?", a: "Because the call depth equals the longest path, so a hundred-by-hundred grid can hit the 1000-frame recursion limit. Moving the stack into a list bounds the depth by memory instead, with the algorithm otherwise unchanged." },
        { q: "Why store walls as bits instead of four booleans?", a: "One int per cell means two grids compare with `==` immediately, copying is trivial, and removing a wall is a single bit operation. More importantly it keeps the tests short, because a cell's whole state is one value." },
        { q: "Why must a wall be removed from both sides?", a: "Because the two cells store the wall between them separately. Remove it on one side and cell A says it can walk to B while B says it cannot come back, so the renderer and any path-finder see different mazes." },
        { q: "What is the seed for, and why not `random.seed()`?", a: "The seed makes results reproducible, which both the tests and any bug report need. Module-level `random.seed()` changes shared state for the whole program, so other code that draws random numbers changes too; `random.Random(seed)` keeps the state to ourselves." },
        { q: "Why split `mazegen` into a package?", a: "So the algorithm can be tested and reused without the drawing or the argument parsing, and so it can be built into a wheel and installed elsewhere. Importing it successfully from another directory is the proof that it really is a package and not a file that happened to sit next door." }
      ]}
    ]
  }
